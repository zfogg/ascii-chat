...
function(ascii_defer_prepare)
    ...
    # Files that are #included by platform stubs (not compiled directly)
    # These must be copied, not transformed, to avoid double-definition errors
    set(_ascii_defer_copy_sources
        "lib/platform/system.c"
        "lib/platform/symbols.c"
    )

    # Directories that must be copied to transformed tree (not transformed, just copied)
    # These are needed for includes but shouldn't be transformed
    ...
    # Copy files that are #included by platform stubs (not compiled directly)
    # These files are included via #include "../file.c" pattern and shouldn't be transformed
    foreach(copy_file IN LISTS _ascii_defer_copy_sources)
        set(src_file "${CMAKE_SOURCE_DIR}/${copy_file}")
        if(EXISTS "${src_file}")
            set(dest_file "${defer_transformed_dir}/${copy_file}")
            ...
            add_custom_command(
                ...
                DEPENDS "${src_file}"
                COMMENT "Copying include-only source ${copy_file}"
                VERBATIM
            )
