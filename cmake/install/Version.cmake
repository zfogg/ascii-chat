...
# Full release archives carry the version data that would otherwise come from
# Git tags. The file is generated by scripts/create-release-tarball.sh and is
# intentionally absent from normal checkouts.
include("${CMAKE_SOURCE_DIR}/cmake/install/SourceArchiveVersion.cmake" OPTIONAL)
...
macro(_get_version_date _tag_name)
    ...
    if(NOT PROJECT_VERSION_DATE AND NOT "${_tag_name}" STREQUAL "")
        execute_process(
...
elseif(NOT \"${ASCIICHAT_SOURCE_COMMIT}\" STREQUAL \"\")
    set(OVERRIDE_VERSION \"${PROJECT_VERSION}\")
endif()
...
if(DEFINED OVERRIDE_VERSION AND NOT OVERRIDE_VERSION STREQUAL \"\")
    if(OVERRIDE_VERSION MATCHES \"^([0-9]+)\\\\.([0-9]+)\\\\.([0-9]+)\$\")
        ...
        elseif(NOT \"${ASCIICHAT_SOURCE_COMMIT}\" STREQUAL \"\")
            set(GIT_COMMIT_HASH \"${ASCIICHAT_SOURCE_COMMIT}\")
...
")
    ...
    if(DEFINED ENV{SOURCE_COMMIT} OR NOT EXISTS "${_GIT_DIR}/HEAD")
        # Source archives have no Git metadata. Their version is supplied at
        # configure time, so only the template can invalidate the header.
