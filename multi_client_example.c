...
void* client_handler_thread(void* arg) {
    ...
    while (!g_should_exit && client->active) {
        ...
        if (sent < 0) {
            log_warn("Client %s:%d disconnected: %s", 
                    client->client_ip, client->port, network_error_string(errno));
            break;
        }
    ...
    log_info("Client handler finished for %s:%d (%lu frames sent)", 
            client->client_ip, client->port, client->frames_sent);
    return NULL;
}
