...
static int verify_conversion(const rgb_t *result, const rgb_t *reference, int pixel_count, int tolerance) {
  int errors = 0;
  for (int i = 0; i < pixel_count; i++) {
    ...
    if (dr > tolerance || dg > tolerance || db > tolerance) {
      if (errors < 10) { // Report first 10 errors
        printf("  Pixel %d mismatch: Result(%d,%d,%d) vs Reference(%d,%d,%d)\n", i, result[i].r, result[i].g,
               result[i].b, reference[i].r, reference[i].g, reference[i].b);
      }
      errors++;
    }
  }
...
static double benchmark_conversion(const char *name, void (*convert_func)(const uint8_t *, rgb_t *, int, int),
                                   const uint8_t *yuy2, rgb_t *rgb, int width, int height, int iterations) {
  printf("\nBenchmarking %s:\n", name);
...
int main(int argc, char **argv) {
  ...
  for (int s = 0; s < num_sizes; s++) {
    ...
    double scalar_time =
        benchmark_conversion("Scalar", convert_yuy2_to_rgb_scalar, yuy2, rgb_scalar, width, height, iterations);

    // Benchmark optimized (auto-dispatch)
    double optimized_time = benchmark_conversion("Optimized (Auto-dispatch)", convert_yuy2_to_rgb_optimized, yuy2,
                                                 rgb_optimized, width, height, iterations);
    ...
    if (yuy2_cpu_has_sse2()) {
      double sse2_time =
          benchmark_conversion("SSE2", convert_yuy2_to_rgb_sse2, yuy2, rgb_sse2, width, height, iterations);
    ...
    if (yuy2_cpu_has_ssse3()) {
      double ssse3_time =
          benchmark_conversion("SSSE3", convert_yuy2_to_rgb_ssse3, yuy2, rgb_ssse3, width, height, iterations);
    ...
    if (yuy2_cpu_has_avx2()) {
      double avx2_time =
          benchmark_conversion("AVX2", convert_yuy2_to_rgb_avx2, yuy2, rgb_avx2, width, height, iterations);
