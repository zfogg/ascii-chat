    ...
    for (int x = 0; x < width; x++) {
      ...
      case 0: // Gradient
        pixels[idx] = (rgb_pixel_t){
            .r = (x * 255) / (width - 1), .g = (y * 255) / (height - 1), .b = ((x + y) * 255) / (width + height - 2)};
        break;
...
Test(video_pipeline, webcam_to_ascii_to_network) {
  ...
  size_t ascii_len =
      ascii_simd_color_neon(test_frame, width * height, ascii_output, sizeof(ascii_output), false, false);
...
Test(video_pipeline, image_frame_to_ascii_conversion) {
  ...
  size_t ascii_len =
      ascii_simd_color_scalar(extracted_pixels, width * height, ascii_output, sizeof(ascii_output), false, false);
...
Test(video_pipeline, different_color_modes_consistency) {
  ...
  size_t fg_len = ascii_simd_color_neon(test_pixels, width * height, fg_output, sizeof(fg_output), false, false);
  size_t bg_len = ascii_simd_color_neon(test_pixels, width * height, bg_output, sizeof(bg_output), true, false);
...
Test(video_pipeline, ascii_compression_in_pipeline) {
  ...
  size_t ascii_len =
      ascii_simd_color_scalar(large_image, width * height, ascii_output, sizeof(ascii_output), false, false);
  cr_assert_gt(ascii_len, 0, "Large ASCII conversion should succeed");

  // Test compression
  uint8_t compressed_buffer[MAX_PACKET_SIZE];
  size_t compressed_size =
      compress_data((uint8_t *)ascii_output, ascii_len, compressed_buffer, sizeof(compressed_buffer));
  ...
  if (compressed_size > 0) {
    ...
    size_t decompressed_size =
        decompress_data(compressed_buffer, compressed_size, decompressed_buffer, sizeof(decompressed_buffer));

    cr_assert_eq(decompressed_size, ascii_len, "Decompressed size should match original");
    cr_assert(memcmp(ascii_output, decompressed_buffer, ascii_len) == 0, "Decompressed data should match original");
  }
...
Test(video_pipeline, pipeline_error_handling) {
  ...
  rgb_pixel_t extreme_pixels[] = {{0, 0, 0}, {255, 255, 255}, {255, 0, 0}, {0, 255, 0}, {0, 0, 255}};
...
Test(video_pipeline, memory_management_in_pipeline) {
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    // ASCII conversion
    char ascii_output[25000];
    size_t ascii_len = ascii_simd_color_scalar(image, width * height, ascii_output, sizeof(ascii_output), false, false);
...
Test(video_pipeline, pipeline_throughput) {
  ...
  log_info("Pipeline throughput: %d frames in %.3f seconds = %.1f FPS (%dx%d)", frame_count, total_time, fps, width,
           height);
...
Test(video_pipeline, ascii_quality_preservation) {
  ...
  size_t ascii_len =
      ascii_simd_color_neon(original_image, width * height, ascii_output, sizeof(ascii_output), false, false);
