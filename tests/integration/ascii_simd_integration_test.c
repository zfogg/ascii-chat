...
static void generate_full_palette_test_image(image_t *test_image, const char *palette) {
  ...
  log_debug("Generating test image (%dx%d) to exercise all %zu palette characters", test_image->w, test_image->h,
            palette_char_count);
  ...
  for (int i = 0; i < verify_count; i++) {
    ...
    log_debug("  pixel[%d]: RGB(%d,%d,%d) -> luminance=%d -> luma_idx=%d -> palette[%d]='%c'", i, pixel.r, pixel.g,
              pixel.b, calc_luma, luma_idx, i, display_char);
  }
...
Test(ascii_simd_integration, monochrome_performance_vs_scalar) {
  ...
  log_debug("Monochrome Performance: Scalar=%.4fms, SIMD=%.4fms, Speedup=%.2fx", (scalar_time / iterations) * 1000,
            (simd_time / iterations) * 1000, speedup);
...
Test(ascii_simd_integration, color_performance_vs_scalar) {
  ...
  log_debug("Color Performance: Scalar=%.4fms, SIMD=%.4fms, Speedup=%.2fx", (scalar_time / iterations) * 1000,
            (simd_time / iterations) * 1000, speedup);
...
Test(ascii_simd_integration, utf8_palette_performance) {
  ...
  log_debug("UTF-8 vs ASCII: ASCII=%.4fms, UTF-8=%.4fms, Penalty=%.2fx", (ascii_time / iterations) * 1000,
            (utf8_time / iterations) * 1000, utf8_penalty);
...
Test(ascii_simd_integration, various_image_sizes_performance) {
  ...
  for (int size_idx = 0; size_idx < num_sizes; size_idx++) {
    ...
    log_debug("%s (%dx%d): Scalar=%.4fms, SIMD=%.4fms, Speedup=%.2fx", test_sizes[size_idx].name, width, height,
...
Test(ascii_simd_integration, simd_vs_scalar_output_consistency) {
  ...
  for (size_t i = 0; i < strlen(scalar_expanded);) {
    ...
    size_t num_found =
        utf8_palette_find_all_char_indices(coverage_pal, &scalar_expanded[i], char_bytes, found_indices, 10);
  ...
  cr_assert_eq(unique_chars_found, (int)palette_char_count, "Must exercise ALL palette positions (%d/%zu found)",
  ...
  for (int p = 0; p < num_palettes; p++) {
    ...
    for (size_t i = 0; i < len;) {
      ...
      if (result[i] == '\033') {
        ...
        while (i < len && result[i] != 'm' && result[i] != 'b')
          i++;
        if (i < len)
          i++;
        continue;
    ...
    log_debug("Palette %d coverage: %d/%zu unique characters found (out of %zu total chars)", p, unique_chars_found,
              unique_palette_chars, palette_char_count);
    cr_assert_eq(unique_chars_found, (int)unique_palette_chars,
                 "Palette %d must exercise ALL unique characters (%d/%zu found)", p, unique_chars_found,
                 unique_palette_chars);
...
Test(ascii_simd_integration, rwlock_concurrency_simulation) {
  ...
  log_debug("Concurrency Test: %d calls in %.3fs (%.4fms each)", iterations, total_time,
...
Test(ascii_simd_integration, extreme_image_sizes) {
  ...
  const struct {
    ...
    int width;
    int height;
  } extreme_sizes[] = {{"Tiny", 1, 1}, {"Narrow", 1, 100}, {"Wide", 100, 1}, {"Large", 640, 480}};
...
Test(ascii_simd_integration, mixed_byte_length_palettes) {
  ...
  } mixed_palettes[] = {// Mix ASCII (1-byte) + 4-byte emojis
...
Test(ascii_simd_integration, mixed_utf8_scalar_faster_than_simd) {
  ...
  const struct {
    ...
    const char *palette;
    const char *description;
  } mixed_palettes[] = {// Mix 1-byte ASCII + 4-byte emojis (worst case for SIMD)
  ...
  for (int p = 0; p < num_palettes; p++) {
    ...
    log_debug("  %s: Scalar=%.4fms, SIMD=%.4fms, Ratio=%.2fx %s", mixed_palettes[p].name,
              (scalar_time / iterations) * 1000, (simd_time / iterations) * 1000, scalar_vs_simd_ratio,
    ...
    for (size_t i = 0; i < test_len; i++) {
      ...
      if (coverage_test[i] == '\033') {
        while (i < test_len && coverage_test[i] != 'm')
          i++;
        continue;
      }
  ...
  if (scalar_wins > total_tests / 2) {
    log_debug("UNEXPECTED: Scalar outperformed SIMD for mixed UTF-8 palettes - this suggests UTF-8 handling complexity "
              "is high");
  } else {
...
Test(ascii_simd_integration, mixed_utf8_output_correctness_mono_and_color) {
  ...
  const struct {
    ...
    const char *palette;
    const char *description;
  } verification_palettes[] = {// Critical test case: Mixed 1-byte + 4-byte (worst case for shuffle mask)
  ...
  const struct {
    ...
    bool is_color;
  } test_modes[] = {{"MONOCHROME", false}, {"COLOR", true}};
  ...
  for (int m = 0; m < 2; m++) {
    ...
    for (int p = 0; p < num_palettes; p++) {
      ...
      log_debug("\n=== %s: %s (%s) ===", test_modes[m].mode_name, verification_palettes[p].name,
...
Test(ascii_simd_integration, neon_monochrome_mixed_byte_comprehensive_performance) {
  ...
  for (int p = 0; p < num_palettes; p++) {
    ...
    for (int s = 0; s < num_sizes; s++) {
      ...
      double scalar_time =
          (scalar_end.tv_sec - scalar_start.tv_sec) + (scalar_end.tv_nsec - scalar_start.tv_nsec) / 1e9;
      ...
      double simd_cold_time =
          (simd_cold_end.tv_sec - simd_cold_start.tv_sec) + (simd_cold_end.tv_nsec - simd_cold_start.tv_nsec) / 1e9;
      ...
      double simd_hot_time =
          (simd_hot_end.tv_sec - simd_hot_start.tv_sec) + (simd_hot_end.tv_nsec - simd_hot_start.tv_nsec) / 1e9;
      ...
      log_debug("  %s (%dx%d): Scalar=%.3fms | SIMD Cold=%.3fms (%.2fx) | Hot=%.3fms (%.2fx) | Cache=%.2fx", size_name,
      ...
      if (hot_speedup < size_adjusted_min) {
        log_debug("    ⚠️  BELOW EXPECTED: Got %.2fx, expected >%.2fx for this palette+size combo", hot_speedup,
                  size_adjusted_min);
      } else {
  ...
  log_debug("   Cache benefits observed: %d/%d tests (%.1f%%)", cache_hit_tests, total_tests,
            (cache_hit_tests * 100.0) / total_tests);
