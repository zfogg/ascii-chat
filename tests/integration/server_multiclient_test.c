...
Test(server_multiclient, image_to_ascii_flow) {
    ...
    if (result == 0) {
        ...
        cr_assert_eq(received_packet.header.type, PACKET_TYPE_ASCII_FRAME,
                    "Received packet should be ASCII frame");
...
Test(server_multiclient, rapid_frame_transmission) {
    ...
    cr_assert_gt(success_rate, 0.7f, "Should successfully send at least 70%% of frames (got %.1f%%)", 
                success_rate * 100);
