...
Test(crypto_network_integration, bidirectional_encrypted_communication) {
    ...
    crypto_result_t result = crypto_create_encrypted_packet(&alice_ctx,
                                                           (const uint8_t*)alice_message, strlen(alice_message),
                                                           alice_encrypted_packet, sizeof(alice_encrypted_packet),
                                                           &alice_packet_len);
    ...
    result = crypto_process_encrypted_packet(&bob_ctx, alice_encrypted_packet, alice_packet_len,
                                            bob_decrypted, sizeof(bob_decrypted), &bob_decrypted_len);
    ...
    result = crypto_create_encrypted_packet(&bob_ctx,
                                           (const uint8_t*)bob_message, strlen(bob_message),
                                           bob_encrypted_packet, sizeof(bob_encrypted_packet),
                                           &bob_packet_len);
    ...
    result = crypto_process_encrypted_packet(&alice_ctx, bob_encrypted_packet, bob_packet_len,
                                            alice_decrypted, sizeof(alice_decrypted), &alice_decrypted_len);
...
Test(crypto_network_integration, password_vs_key_exchange_priority) {
    ...
    result = crypto_encrypt(&ctx, (const uint8_t*)test_message, strlen(test_message),
    ...
    result = crypto_decrypt(&peer_ctx, ciphertext, ciphertext_len,
...
Test(crypto_network_integration, multiple_messages_same_session) {
    ...
    const char *messages[] = {
        ...
        "Message 2: Sending video data",
        "Message 3: Audio stream active",
        "Message 4: Client count update",
    ...
    for (int i = 0; i < 5; i++) {
        ...
        size_t packet_len;
        crypto_result_t result = crypto_create_encrypted_packet(&client_ctx,
        ...
        uint8_t decrypted[1024];
        size_t decrypted_len;
        result = crypto_process_encrypted_packet(&server_ctx, encrypted_packet, packet_len,
                                                decrypted, sizeof(decrypted), &decrypted_len);
        cr_assert_eq(result, CRYPTO_OK, "Message %d decryption should succeed", i + 1);
        cr_assert_eq(memcmp(decrypted, messages[i], strlen(messages[i])), 0,
        ...
        result = crypto_create_encrypted_packet(&server_ctx,
                                               (const uint8_t*)ack_message, strlen(ack_message),
                                               encrypted_packet, sizeof(encrypted_packet), &packet_len);
        cr_assert_eq(result, CRYPTO_OK, "ACK %d encryption should succeed", i + 1);

        result = crypto_process_encrypted_packet(&client_ctx, encrypted_packet, packet_len,
                                                decrypted, sizeof(decrypted), &decrypted_len);
        cr_assert_eq(result, CRYPTO_OK, "ACK %d decryption should succeed", i + 1);
        cr_assert_eq(memcmp(decrypted, ack_message, strlen(ack_message)), 0,
                     "ACK %d content should match", i + 1);
    }
...
Test(crypto_network_integration, large_message_handling) {
    ...
    crypto_result_t result = crypto_create_encrypted_packet(&ctx1, large_message, large_message_size,
                                                           encrypted_packet, large_message_size + 1024, &packet_len);
    ...
    result = crypto_process_encrypted_packet(&ctx2, encrypted_packet, packet_len,
                                            decrypted_message, large_message_size, &decrypted_len);
...
Test(crypto_network_integration, error_handling_integration) {
    ...
    crypto_result_t result = crypto_create_encrypted_packet(&ctx1, (const uint8_t*)test_msg, strlen(test_msg),
    ...
    result = crypto_create_encrypted_packet(&ctx1, (const uint8_t*)test_msg, strlen(test_msg),
                                           packet, sizeof(packet), &packet_len);
    ...
    result = crypto_process_encrypted_packet(&ctx2, malformed_packet, sizeof(malformed_packet),
                                            decrypted, sizeof(decrypted), &decrypted_len);
...
Test(crypto_network_integration, session_cleanup_and_restart) {
    ...
    crypto_result_t result = crypto_create_encrypted_packet(&ctx1, (const uint8_t*)msg1, strlen(msg1),
                                                           packet, sizeof(packet), &packet_len);
    ...
    result = crypto_create_encrypted_packet(&ctx1, (const uint8_t*)msg2, strlen(msg2),
                                           packet, sizeof(packet), &packet_len);
