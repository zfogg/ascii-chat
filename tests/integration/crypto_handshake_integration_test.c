...
Test(crypto_handshake_integration, complete_handshake_flow) {
    ...
    // Verify final states
    cr_assert_eq(server_ctx.state, CRYPTO_HANDSHAKE_COMPLETE, "Server should be complete");
...
static key_type_test_case_t key_type_cases[] = {
    ...
    },
    {
        ...
        "github:username",
        KEY_TYPE_ED25519,
        "GitHub username (should fetch Ed25519)"
    },
    {
        "gitlab",
        "gitlab:username",
        KEY_TYPE_ED25519,
        "GitLab username (should fetch Ed25519)"
    }
};
...
Test(crypto_handshake_integration, encryption_after_handshake) {
    ...
    // Server encrypts
    int encrypt_result = crypto_handshake_encrypt(&server_ctx, (const uint8_t*)plaintext, strlen(plaintext), ciphertext, &ciphertext_len);
    ...
    // Client decrypts
    int decrypt_result = crypto_handshake_decrypt(&client_ctx, ciphertext, ciphertext_len, decrypted, &decrypted_len);
...
Test(crypto_handshake_integration, bidirectional_encryption) {
    ...
    int server_encrypt = crypto_handshake_encrypt(&server_ctx, (const uint8_t*)server_message, strlen(server_message), server_cipher, &server_cipher_len);
    cr_assert_eq(server_encrypt, 0, "Server encryption should succeed");

    // Client decrypts server message
    int client_decrypt = crypto_handshake_decrypt(&client_ctx, server_cipher, server_cipher_len, client_decrypted, &client_decrypted_len);
    cr_assert_eq(client_decrypt, 0, "Client decryption should succeed");
    cr_assert_eq(memcmp(client_decrypted, server_message, strlen(server_message)), 0, "Client should decrypt server message correctly");

    // Client encrypts to server
    int client_encrypt = crypto_handshake_encrypt(&client_ctx, (const uint8_t*)client_message, strlen(client_message), client_cipher, &client_cipher_len);
    cr_assert_eq(client_encrypt, 0, "Client encryption should succeed");

    // Server decrypts client message
    int server_decrypt = crypto_handshake_decrypt(&server_ctx, client_cipher, client_cipher_len, server_decrypted, &server_decrypted_len);
    cr_assert_eq(server_decrypt, 0, "Server decryption should succeed");
    cr_assert_eq(memcmp(server_decrypted, client_message, strlen(client_message)), 0, "Server should decrypt client message correctly");
...
Theory((const char* auth_method, bool known_hosts_verification, bool client_whitelist_check), crypto_handshake_integration, authentication_scenarios) {
    setup_mock_network();
    ...
    if (known_hosts_verification) {
        ...
        add_known_host("test-server.com", "8080", "x25519", "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef");
    }
...
Test(crypto_handshake_integration, concurrent_handshakes) {
    ...
    for (int i = 0; i < num_clients; i++) {
        cr_assert(client_args[i].result == 0 || client_args[i].result == -1, "Client %d should complete or fail gracefully", i);
    }
...
Test(crypto_handshake_integration, large_data_encryption) {
    ...
    // Encrypt large data
    int encrypt_result = crypto_handshake_encrypt(&server_ctx, large_data, large_size, ciphertext, &ciphertext_len);
