...
Test(network, set_socket_timeout_valid) {
    ...
    int result = set_socket_timeout(sockfd, 5);
...
Test(network, set_socket_timeout_invalid_socket) {
    int result = set_socket_timeout(-1, 5);
...
Test(network, connect_with_timeout_invalid_socket) {
    ...
    bool result = connect_with_timeout(-1, (struct sockaddr *)&addr, sizeof(addr), 5);
    cr_assert_eq(result, false);
}

Test(network, send_with_timeout_invalid_socket) {
    const char *data = "test data";
    ssize_t result = send_with_timeout(-1, data, strlen(data), 5);
    cr_assert_eq(result, -1);
}

Test(network, recv_with_timeout_invalid_socket) {
    char buffer[1024];
    ssize_t result = recv_with_timeout(-1, buffer, sizeof(buffer), 5);
    cr_assert_eq(result, -1);
}
