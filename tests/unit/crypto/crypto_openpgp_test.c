 ...
 */
static const uint8_t OLD_FORMAT_PUBKEY_PACKET[] = {
    0x98, 0x33, // Old format header: tag 6, length 51
    0x04,       // Version 4
    ...
    0x40,
    0x39, 0xAC, 0xA4, 0x20, 0xCC, 0x9A, 0x42, 0x2F,
    0x02, 0x05, 0x33, 0x62, 0x17, 0xDA, 0x3F, 0x35,
    0xB9, 0xBA, 0x2F, 0x90, 0xF0, 0x47, 0xD8, 0x75,
    0x99, 0xA4, 0xB7, 0xCA, 0xA1, 0xB9, 0x3C, 0x53
};
 ...
 */
static const uint8_t NEW_FORMAT_PUBKEY_PACKET[] = {
    0xC6, 0x33, // New format header: tag 6, length 51
    0x04,       // Version 4
    0x69, 0x64, 0x0B, 0x39, // Created timestamp
    0x16,       // Algorithm 22 (EdDSA)
    // OID for Ed25519
 ...
 */
static const uint8_t EXPECTED_OLD_FORMAT_KEY[32] = {
    0x39, 0xAC, 0xA4, 0x20, 0xCC, 0x9A, 0x42, 0x2F,
    0x02, 0x05, 0x33, 0x62, 0x17, 0xDA, 0x3F, 0x35,
    0xB9, 0xBA, 0x2F, 0x90, 0xF0, 0x47, 0xD8, 0x75,
    0x99, 0xA4, 0xB7, 0xCA, 0xA1, 0xB9, 0x3C, 0x53
 ...
 */
static const uint8_t EXPECTED_NEW_FORMAT_KEY[32] = {
    0xDE, 0xAD, 0xBE, 0xEF, 0x00, 0x11, 0x22, 0x33,
    0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB,
    0xCC, 0xDD, 0xEE, 0xFF, 0x01, 0x23, 0x45, 0x67,
    0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98
...
Test(openpgp, parse_old_format_packet_header) {
    ...
    asciichat_error_t result = openpgp_parse_packet_header(
        OLD_FORMAT_PUBKEY_PACKET,
        sizeof(OLD_FORMAT_PUBKEY_PACKET),
        &header
    );
...
Test(openpgp, parse_new_format_packet_header) {
    openpgp_packet_header_t header;
    asciichat_error_t result = openpgp_parse_packet_header(
        NEW_FORMAT_PUBKEY_PACKET,
        sizeof(NEW_FORMAT_PUBKEY_PACKET),
        &header
    );
...
Test(openpgp, parse_old_format_pubkey_packet) {
    ...
    cr_assert(memcmp(pubkey.pubkey, EXPECTED_OLD_FORMAT_KEY, 32) == 0,
              "Should extract correct Ed25519 public key");
}
...
Test(openpgp, parse_new_format_pubkey_packet) {
    ...
    cr_assert_eq(pubkey.version, 4, "Should extract version 4");
    cr_assert_eq(pubkey.algorithm, 22, "Should extract EdDSA algorithm");
    cr_assert(memcmp(pubkey.pubkey, EXPECTED_NEW_FORMAT_KEY, 32) == 0,
              "Should extract correct Ed25519 public key");
}
...
Test(openpgp, decode_base64_with_newlines) {
    const char *base64_with_newlines =
        "bURFYVd4Q09SWUpLd1lCQkFIYVJ3OEJBUWR\n"
    ...
    uint8_t *binary_out;
    size_t binary_len;
    asciichat_error_t result = openpgp_base64_decode(
        base64_with_newlines,
        strlen(base64_with_newlines),
        &binary_out,
        &binary_len
    );
