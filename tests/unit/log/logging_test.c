...
Test(logging, log_file_operations) {
  ...
  log_init(test_log_file, LOG_DEBUG, false, false);
...
Test(logging, log_file_initialization_failure) {
  ...
  log_init(invalid_file, LOG_INFO, false, false);
...
Test(logging, log_file_reinitialization) {
  ...
  log_init(test_log_file1, LOG_INFO, false, false);
  log_info("Message to first file");

  // Reinitialize with second file
  log_init(test_log_file2, LOG_DEBUG, false, false);
...
ParameterizedTest(log_init_test_case_t *tc, logging, log_initialization_variations) {
  log_init(NULL, tc->level, false, false);
...
Test(logging, log_level_env_string_values) {
  ...
  log_destroy();
  log_init(test_log_file, LOG_FATAL, false, false); // Should be overridden by env var
  log_set_terminal_output(false);
  ...
  log_destroy();
  log_init(test_log_file, LOG_FATAL, false, false);
  log_set_terminal_output(false);
...
Test(logging, log_level_env_unset_uses_default) {
  ...
  safe_setenv("LOG_LEVEL", NULL);
  log_destroy();
  log_init(test_log_file, LOG_WARN, false, false); // Should use this level
  log_set_terminal_output(false);
...
Test(logging, log_level_env_invalid_uses_default) {
  ...
  log_destroy();
  log_init(test_log_file, LOG_INFO, false, false); // Should use default (INFO) when env is invalid
  log_set_terminal_output(false);
...
Test(logging, log_level_env_dos_protection) {
  ...
  log_init(test_log_file, LOG_INFO, false, false);
  log_set_terminal_output(false);
...
Test(logging, log_level_env_before_init) {
  ...
  // Now properly init
  log_init(test_log_file, LOG_FATAL, false, false); // Should still respect LOG_LEVEL=DEBUG
