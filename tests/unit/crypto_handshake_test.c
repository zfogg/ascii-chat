...
static ssize_t mock_socket_send(int sock, const void* buf, size_t len, int flags) {
    ...
    if (!mock_sock || !mock_sock->connected) return -1;
...
Test(crypto_handshake, server_complete) {
    ...
    cr_assert_eq(ctx.state, CRYPTO_HANDSHAKE_COMPLETE, "State should be COMPLETE");
...
Test(crypto_handshake, state_machine_progression) {
    ...
    cr_assert_eq(ctx.state, CRYPTO_HANDSHAKE_COMPLETE, "Should be in COMPLETE state");
