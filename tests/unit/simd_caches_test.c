...
Test(simd_caches, animation_palette_cycling_realistic) {
    ...
    cr_assert_geq(survived_count, num_palettes / 2,
                 "At least half of animation palettes should survive (%d/%d)",
...
Test(simd_caches, old_frequent_palette_persistence) {
    ...
    for (int i = 0; i < high_frequency_accesses; i++) {
        ...
        if (i % 20 == 0) {
            printf("Building frequency: access %d/100\n", i);
        }
    }
    ...
    if (old_cache) {
        cr_assert_lt(access_time, 0.001,
                    "Old frequent palette should be cached (got %.6fs)", access_time);
    }
}
...
Test(simd_caches, eviction_ordering_verification) {
    ...
    cr_assert_lt(survivor_time, victim_time * 2.0 + 0.001,
                 "Survivor (%.6fs) should be faster than victim (%.6fs)", 
...
Test(simd_caches, min_heap_ordering_verification) {
    ...
    for (int e = 0; e < num_entries; e++) {
        printf("Forcing score update for %s\n", heap_test_entries[e].name);

        for (int update = 0; update < 10; update++) {
            utf8_palette_cache_t *cache = get_utf8_palette_cache(heap_test_entries[e].palette);
            cr_assert_not_null(cache, "%s score update %d should succeed",
                              heap_test_entries[e].name, update);
        }
    }
    ...
    for (int pressure = 0; pressure < 10; pressure++) {
        ...
        printf("Created pressure palette %d\n", pressure);
    }
...
Test(simd_caches, heap_score_calculation_accuracy) {
    ...
    for (int t = 0; t < num_test_cases; t++) {
        ...
        for (int access = 0; access < score_test_cases[t].access_count; access++) {
            ...
            cr_assert_not_null(score_caches[t], "%s access %d should succeed",
                              score_test_cases[t].name, access);
        }
    ...
    for (int t = 0; t < num_test_cases; t++) {
        ...
        if (strcmp(score_test_cases[t].name, "WORST") == 0) worst_survived = survived;
        if (strcmp(score_test_cases[t].name, "EXCELLENT") == 0) excellent_survived = survived;
    }
