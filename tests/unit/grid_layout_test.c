...
static void calculate_optimal_grid(int terminal_width, int terminal_height, int num_clients, int *out_cols,
  ...
  if (num_clients == 2) {
    ...
    float terminal_aspect = (float)terminal_width / (float)terminal_height;
...
Test(grid_layout, two_clients_horizontal_vs_vertical) {
  ...
  // Tall terminal - should prefer 1x2 (stacked)
  calculate_optimal_grid(80, 48, 2, &cols, &rows, &cell_width, &cell_height);
  cr_expect_eq(cols, 1, "Tall terminal with 2 clients should use 1 column");
  cr_expect_eq(rows, 2, "Tall terminal with 2 clients should use 2 rows");
  cr_expect_eq(cell_width, 80, "Each cell should be full width");
  cr_expect_eq(cell_height, 24, "Each cell should be half height");
  // Assert full space utilization
  cr_expect_eq(cols * cell_width, 80, "Should use all horizontal space");
...
Test(grid_layout, three_clients_optimal) {
  ...
  // Tall terminal with 3 clients - try 1x3
  calculate_optimal_grid(80, 72, 3, &cols, &rows, &cell_width, &cell_height);
  cr_expect_eq(cols, 1, "Tall terminal with 3 clients should use 1x3");
  cr_expect_eq(rows, 3);
  cr_expect_eq(cell_width, 80);
  cr_expect_eq(cell_height, 24);
  // Assert full space utilization
  cr_expect_eq(cols * cell_width, 80, "Should use all horizontal space");
...
Test(grid_layout, four_clients_2x2) {
  ...
  // Standard terminal with 4 clients - perfect 2x2
  calculate_optimal_grid(120, 48, 4, &cols, &rows, &cell_width, &cell_height);
  cr_expect_eq(cols, 2, "4 clients should use 2x2 grid");
  cr_expect_eq(rows, 2);
  cr_expect_eq(cell_width, 60);
  cr_expect_eq(cell_height, 24);
  // Assert full space utilization
  ...
  // Wide terminal with 4 clients - might prefer 4x1 if wide enough
  calculate_optimal_grid(240, 24, 4, &cols, &rows, &cell_width, &cell_height);
  cr_expect_eq(cols, 4, "Very wide terminal with 4 clients could use 4x1");
  cr_expect_eq(rows, 1);
  // Assert full space utilization
  cr_expect_eq(cols * cell_width, 240, "Should use all horizontal space");
  cr_expect_eq(rows * cell_height, 24, "Should use all vertical space");

  // Tall terminal with 4 clients - might prefer 1x4 if tall enough
  calculate_optimal_grid(80, 96, 4, &cols, &rows, &cell_width, &cell_height);
  cr_expect_eq(cols, 1, "Very tall terminal with 4 clients could use 1x4");
  cr_expect_eq(rows, 4);
  // Assert full space utilization
  cr_expect_eq(cols * cell_width, 80, "Should use all horizontal space");
...
Test(grid_layout, five_to_six_clients) {
  ...
  // 5 clients - should use 3x2 or 2x3 depending on terminal
  calculate_optimal_grid(180, 48, 5, &cols, &rows, &cell_width, &cell_height);
  cr_expect_eq(cols, 3, "Wide terminal with 5 clients should use 3x2");
  cr_expect_eq(rows, 2);
  // Assert full space utilization
  cr_expect_eq(cols * cell_width, 180, "Should use all horizontal space");
  cr_expect_eq(rows * cell_height, 48, "Should use all vertical space");
  ...
  // 6 clients - perfect 3x2 or 2x3
  calculate_optimal_grid(180, 48, 6, &cols, &rows, &cell_width, &cell_height);
  cr_expect_eq(cols, 3, "6 clients should use 3x2 for wide terminal");
  cr_expect_eq(rows, 2);
  // Assert full space utilization
  cr_expect_eq(cols * cell_width, 180, "Should use all horizontal space");
  cr_expect_eq(rows * cell_height, 48, "Should use all vertical space");
...
Test(grid_layout, seven_to_nine_clients) {
  ...
  // 9 clients - perfect 3x3
  calculate_optimal_grid(150, 60, 9, &cols, &rows, &cell_width, &cell_height);
  cr_expect_eq(cols, 3, "9 clients should use perfect 3x3 grid");
  cr_expect_eq(rows, 3);
  cr_expect_eq(cell_width, 50);
...
Test(grid_layout, aspect_ratio_preferences) {
  ...
  // Option 1: 4x1 = cells 50x50 (aspect 1.0) - bad
  // Option 2: 2x2 = cells 100x25 (aspect 4.0) - worse
  // Option 3: 1x4 = cells 200x12.5 (aspect 16.0) - terrible
  calculate_optimal_grid(200, 50, 4, &cols, &rows, &cell_width, &cell_height);
  // Should pick 4x1 as it's closest to target aspect of 2.0
  cr_expect_eq(cols, 4, "Should pick 4x1 for best aspect ratio");
  cr_expect_eq(rows, 1);
  // Assert full space utilization
  cr_expect_eq(cols * cell_width, 200, "Should use all horizontal space");
  cr_expect_eq(rows * cell_height, 50, "Should use all vertical space");
...
static grid_test_case_t test_cases[] = {
    ...
    {80, 24, 1, 1, 1, 80, 24},   // Single client standard terminal
    {160, 48, 1, 1, 1, 160, 48}, // Single client large terminal
    {120, 30, 2, 2, 1, 60, 15},  // Two clients wide terminal (2x1 layout for aspect ratio)
    {60, 40, 2, 2, 1, 30, 20},   // Two clients tall terminal (2x1 layout for aspect ratio)
    {90, 30, 3, 2, 2, 45, 15},   // Three clients wide terminal (2x2 for better aspect)
    {60, 60, 3, 1, 3, 60, 20},   // Three clients tall terminal (1x3 layout)
    {120, 40, 3, 2, 2, 60, 20},  // Three clients medium terminal (2x2)
    {100, 40, 4, 3, 2, 33, 20},  // Four clients standard (3x2 for better aspect)
    {160, 40, 4, 4, 1, 40, 10},  // Four clients wide (4x1 for aspect)
    {80, 80, 4, 2, 2, 40, 40},   // Four clients square (2x2)
    {150, 50, 5, 3, 2, 50, 25},  // Five clients (3x2)
    {120, 60, 6, 3, 2, 40, 30},  // Six clients standard (3x2 for aspect)
    {180, 60, 6, 3, 2, 60, 30},  // Six clients wide (3x2)
    {150, 60, 9, 4, 3, 37, 20},  // Nine clients max (4x3 for aspect)
};
...
ParameterizedTest(grid_test_case_t *tc, grid_layout, parameterized_comprehensive) {
  ...
  // Assert full space utilization for all parameterized tests
  cr_expect_eq(cols * cell_width, tc->terminal_width,
               "Terminal %dx%d, %d clients: Should use all horizontal space (got %d, expected %d)", tc->terminal_width,
               tc->terminal_height, tc->num_clients, cols * cell_width, tc->terminal_width);
  cr_expect_eq(rows * cell_height, tc->terminal_height,
               "Terminal %dx%d, %d clients: Should use all vertical space (got %d, expected %d)", tc->terminal_width,
               tc->terminal_height, tc->num_clients, rows * cell_height, tc->terminal_height);
}
