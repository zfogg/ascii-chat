...
Test(youtube, url_detection) {
  ...
  for (size_t i = 0; i < num_cases; i++) {
    ...
    cr_assert_eq(result, tc->expected_is_youtube, "%s: youtube_is_youtube_url(\"%s\") should return %s",
...
static video_id_extraction_test_case_t video_id_cases[] = {
    // Valid video IDs
    {"https://youtube.com/watch?v=dQw4w9WgXcQ", "dQw4w9WgXcQ", ASCIICHAT_OK, "Standard YouTube URL"},
    {"https://youtu.be/dQw4w9WgXcQ", "dQw4w9WgXcQ", ASCIICHAT_OK, "YouTube short URL"},
    {"https://m.youtube.com/watch?v=dQw4w9WgXcQ", "dQw4w9WgXcQ", ASCIICHAT_OK, "Mobile YouTube URL"},
    {"https://youtube.com/watch?v=dQw4w9WgXcQ&t=30", "dQw4w9WgXcQ", ASCIICHAT_OK, "YouTube URL with parameters"},

    // Invalid URLs
    {"https://example.com/watch?v=ID", "InvalidVideoId", ERROR_YOUTUBE_INVALID_URL, "Non-YouTube domain"},
    {"https://youtube.com/watch", "InvalidVideoId", ERROR_YOUTUBE_INVALID_URL, "YouTube URL without video ID"},
};
...
Test(youtube, video_id_extraction) {
  ...
  for (size_t i = 0; i < num_cases; i++) {
    ...
    cr_assert_eq(result, tc->expected_error, "%s: youtube_extract_video_id(\"%s\") should return %d", tc->description,
                 tc->url, tc->expected_error);

    if (result == ASCIICHAT_OK) {
      cr_assert_str_eq(extracted_id, tc->expected_video_id, "%s: Extracted video ID should be '%s', got '%s'",
...
Test(youtube, small_buffer_extraction) {
  ...
  asciichat_error_t result =
      youtube_extract_video_id("https://youtube.com/watch?v=dQw4w9WgXcQ", small_id, sizeof(small_id));
  cr_assert_eq(result, ERROR_INVALID_PARAM, "Small buffer should return ERROR_INVALID_PARAM");
}

Test(youtube, video_id_length) {
  // Verify video ID is 11 characters long for valid IDs
  char id[16] = {0};
  asciichat_error_t result = youtube_extract_video_id("https://youtube.com/watch?v=dQw4w9WgXcQ", id, sizeof(id));
...
Test(youtube, youtube_url_variations) {
  ...
  const char *valid_urls[] = {
      ...
      "https://m.youtube.com/watch?v=dQw4w9WgXcQ",   "https://youtu.be/dQw4w9WgXcQ",
      "http://youtube.com/watch?v=dQw4w9WgXcQ",
  };

  for (size_t i = 0; i < sizeof(valid_urls) / sizeof(valid_urls[0]); i++) {
    cr_assert(youtube_is_youtube_url(valid_urls[i]), "URL %zu should be recognized as YouTube URL: %s", i,
              valid_urls[i]);

    char id[16] = {0};
    asciichat_error_t result = youtube_extract_video_id(valid_urls[i], id, sizeof(id));
    cr_assert_eq(result, ASCIICHAT_OK, "Video ID extraction should succeed for URL %zu: %s", i, valid_urls[i]);
    cr_assert_str_eq(id, "dQw4w9WgXcQ", "All URL formats should extract same video ID, got '%s' for URL %zu", id, i);
  }
}
...
Test(youtube, error_code_definitions) {
  ...
  int codes[] = {ERROR_YOUTUBE_INVALID_URL, ERROR_YOUTUBE_EXTRACT_FAILED, ERROR_YOUTUBE_UNPLAYABLE,
  ...
  for (size_t i = 0; i < sizeof(codes) / sizeof(codes[0]); i++) {
    ...
    cr_assert_not_null(str, "Error code %d should have a string representation", codes[i]);
    cr_assert(strlen(str) > 0, "Error code %d string should not be empty", codes[i]);
  }
}
...
Test(youtube, error_codes_distinct) {
  ...
  for (size_t i = 0; i < sizeof(codes) / sizeof(codes[0]); i++) {
    for (size_t j = i + 1; j < sizeof(codes) / sizeof(codes[0]); j++) {
      cr_assert_neq(codes[i], codes[j], "YouTube error codes at index %zu and %zu should be distinct", i, j);
    }
  }
}
