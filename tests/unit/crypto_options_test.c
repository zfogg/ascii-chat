...
static crypto_options_test_case_t crypto_options_cases[] = {
    ...
    {
        "Disable encryption",
        2, {"program", "--no-encrypt"},
        true, true, false, false, false, false,
        NULL, NULL, NULL, NULL, 0
    },
    {
        "Set password key",
        3, {"program", "--key", "mypassword"},
        true, false, true, false, false, false,
        "mypassword", NULL, NULL, NULL, 0
    },
    {
        "Set SSH key file",
        3, {"program", "--ssh-key", "~/.ssh/id_ed25519"},
        true, false, false, true, false, false,
        NULL, "~/.ssh/id_ed25519", NULL, NULL, 0
    },
    {
        "Set server key file (server only)",
        3, {"program", "--server-key", "/etc/ascii-chat/server_key"},
        false, false, false, false, true, false,
        NULL, NULL, "/etc/ascii-chat/server_key", NULL, 0
    },
    {
        "Set client keys file (server only)",
        3, {"program", "--client-keys", "/etc/ascii-chat/authorized_keys"},
        false, false, false, false, false, true,
        NULL, NULL, NULL, "/etc/ascii-chat/authorized_keys", 0
    },
    {
        "Multiple crypto options",
        5, {"program", "--no-encrypt", "--key", "password", "--ssh-key", "~/.ssh/id_ed25519"},
        true, true, true, true, false, false,
        "password", "~/.ssh/id_ed25519", NULL, NULL, 0
    },
    {
        "GitHub key reference",
        3, {"program", "--key", "github:username"},
        true, false, true, false, false, false,
        "github:username", NULL, NULL, NULL, 0
    },
    {
        "GitLab key reference",
        3, {"program", "--key", "gitlab:username"},
        true, false, true, false, false, false,
        "gitlab:username", NULL, NULL, NULL, 0
    },
    {
        "GPG key reference",
        3, {"program", "--key", "gpg:0x1234567890ABCDEF"},
        true, false, true, false, false, false,
        "gpg:0x1234567890ABCDEF", NULL, NULL, NULL, 0
    },
    {
        "Raw X25519 key",
        3, {"program", "--key", "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"},
        true, false, true, false, false, false,
        "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef", NULL, NULL, NULL, 0
    },
    {
        "SSH Ed25519 key",
        3, {"program", "--key", "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGplY2VrZXJzIGVkMjU1MTkga2V5"},
        true, false, true, false, false, false,
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGplY2VrZXJzIGVkMjU1MTkga2V5", NULL, NULL, NULL, 0
    },
    {
        "Long password key",
        3, {"program", "--key", "very-long-password-with-special-chars!@#$%^&*()"},
        true, false, true, false, false, false,
        "very-long-password-with-special-chars!@#$%^&*()", NULL, NULL, NULL, 0
    },
    {
...
ParameterizedTest(crypto_options_test_case_t *tc, crypto_options, crypto_options_parsing_tests) {
    ...
    cr_assert_eq(opt_key != NULL, tc->expect_key_set, "Key should be set for case: %s", tc->description);
    cr_assert_eq(opt_ssh_key != NULL, tc->expect_ssh_key_set, "SSH key should be set for case: %s", tc->description);
    cr_assert_eq(opt_server_key != NULL, tc->expect_server_key_set, "Server key should be set for case: %s", tc->description);
    cr_assert_eq(opt_client_keys != NULL, tc->expect_client_keys_set, "Client keys should be set for case: %s", tc->description);

    if (tc->expected_key) {
        cr_assert_not_null(opt_key, "Key should not be NULL for case: %s", tc->description);
        cr_assert_str_eq(opt_key, tc->expected_key, "Key should match for case: %s", tc->description);
    }

    if (tc->expected_ssh_key) {
        cr_assert_not_null(opt_ssh_key, "SSH key should not be NULL for case: %s", tc->description);
        cr_assert_str_eq(opt_ssh_key, tc->expected_ssh_key, "SSH key should match for case: %s", tc->description);
    }

    if (tc->expected_server_key) {
        cr_assert_not_null(opt_server_key, "Server key should not be NULL for case: %s", tc->description);
    ...
    if (tc->expected_client_keys) {
        cr_assert_not_null(opt_client_keys, "Client keys should not be NULL for case: %s", tc->description);
        cr_assert_str_eq(opt_client_keys, tc->expected_client_keys, "Client keys should match for case: %s", tc->description);
    }
}
...
Test(crypto_options, client_only_options) {
    ...
    cr_assert_null(opt_server_key, "Server key should not be set for client");
}
...
Test(crypto_options, server_only_options) {
    ...
    cr_assert_not_null(opt_client_keys, "Client keys should be set for server");
...
Test(crypto_options, mutually_exclusive_options) {
    ...
    cr_assert_not_null(opt_key, "Key should still be set");
}
...
Test(crypto_options, invalid_key_formats) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        cr_assert_not_null(opt_key, "Key should be set even for invalid format: %s", invalid_keys[i]);
        cr_assert_str_eq(opt_key, invalid_keys[i], "Key should match input: %s", invalid_keys[i]);
    }
}
...
Test(crypto_options, very_long_key_value) {
    ...
    cr_assert_not_null(opt_key, "Long key should be accepted");
    cr_assert_str_eq(opt_key, long_key, "Long key should match input");
}
...
Test(crypto_options, special_characters_in_key) {
    ...
    cr_assert_not_null(opt_key, "Special characters should be accepted");
    cr_assert_str_eq(opt_key, special_key, "Special characters should be preserved");
}
...
Test(crypto_options, unicode_characters_in_key) {
    ...
    cr_assert_not_null(opt_key, "Unicode characters should be accepted");
    cr_assert_str_eq(opt_key, unicode_key, "Unicode characters should be preserved");
}
...
Test(crypto_options, multiple_ssh_keys) {
    ...
    cr_assert_not_null(opt_ssh_key, "SSH key should be set");
    cr_assert_str_eq(opt_ssh_key, "~/.ssh/id_ed25519_2", "Should use last SSH key");
}
...
Test(crypto_options, empty_arguments) {
    ...
    cr_assert_null(opt_key, "Key should be NULL by default");
    cr_assert_null(opt_ssh_key, "SSH key should be NULL by default");
}
...
Theory((bool is_client, bool no_encrypt, bool has_key, bool has_ssh_key), crypto_options, option_combinations) {
    ...
    cr_assert_eq(opt_key != NULL, has_key, "Key should be set if specified");
    cr_assert_eq(opt_ssh_key != NULL, has_ssh_key, "SSH key should be set if specified");
}
...
Test(crypto_options, file_path_expansion) {
    ...
    cr_assert_not_null(opt_ssh_key, "SSH key should be set");
    cr_assert_str_eq(opt_ssh_key, "~/.ssh/id_ed25519", "SSH key path should match");
}
...
Test(crypto_options, absolute_file_paths) {
    ...
    cr_assert_not_null(opt_server_key, "Server key should be set");
...
Test(crypto_options, relative_file_paths) {
    ...
    cr_assert_not_null(opt_client_keys, "Client keys should be set");
...
Test(crypto_options, many_options) {
    ...
    cr_assert(opt_no_encrypt, "No encrypt should be set");
    cr_assert_not_null(opt_key, "Key should be set");
    cr_assert_not_null(opt_ssh_key, "SSH key should be set");
    cr_assert_not_null(opt_server_key, "Server key should be set");
    cr_assert_not_null(opt_client_keys, "Client keys should be set");
}
...
Test(crypto_options, repeated_options) {
    ...
    cr_assert_not_null(opt_key, "Key should be set");
    cr_assert_str_eq(opt_key, "second-key", "Should use last key");
    cr_assert_not_null(opt_ssh_key, "SSH key should be set");
    cr_assert_str_eq(opt_ssh_key, "second-ssh", "Should use last SSH key");
}
