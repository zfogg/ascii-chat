...
TestSuite(ansi_fast, .init = setup_quiet_test_logging, .fini = restore_test_logging);
...
Test(ansi_fast, append_truecolor_fg_basic) {
    ...
    *result = '\0'; // Null terminate for string comparison
    ...
    cr_assert_eq(result - buffer, 19, "Result pointer should be positioned correctly");
}
...
Test(ansi_fast, append_truecolor_fg_bg_combined) {
    ...
    *result = '\0';
    ...
    cr_assert_eq(result - buffer, 30, "Result pointer should be positioned correctly");
}
...
Test(ansi_fast, ansi_rle_finish) {
    ...
    cr_assert_eq(buffer[ctx.length - 1], '\0', "Buffer should be null terminated");
...
Test(ansi_fast, rgb_to_16color_dithered_basic) {
    ...
    // Test basic dithering
    result = rgb_to_16color_dithered(255, 0, 0, 0, 0, 10, 10, error_buffer);
    cr_assert_eq(result, 9, "Red should map to bright red");
    
    // Check that error was distributed
    cr_assert_neq(error_buffer[1].r, 0, "Error should be distributed to right pixel");
    cr_assert_neq(error_buffer[10].r, 0, "Error should be distributed to bottom pixel");
}
...
Test(ansi_fast, performance_rle_generation) {
    ...
    cr_assert_eq(buffer[ctx.length - 1], '\0', "Should be null terminated");
}
...
Test(ansi_fast, integration_full_workflow) {
    ...
    cr_assert(strstr(buffer, "REDB") != NULL, "Should contain characters");
}
