...
TestSuite(webcam_platform, .init = setup_quiet_test_logging, .fini = restore_test_logging);
...
Test(webcam_platform, get_platform) {
    ...
    cr_assert(platform == WEBCAM_PLATFORM_V4L2 ||
              platform == WEBCAM_PLATFORM_AVFOUNDATION ||
...
Test(webcam_platform, platform_name_all_values) {
    ...
    webcam_platform_type_t platforms[] = {
        WEBCAM_PLATFORM_UNKNOWN,
