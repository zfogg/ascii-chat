...
export function parseAsciiFrame(payload: Uint8Array): AsciiFrame {
  console.log(
    `[AsciiFrameParser] parseAsciiFrame() called with payload size: ${payload.length}`,
  );
  ...
  console.log(
    `[AsciiFrameParser]   offset 12 (compressedSize): ${compressedSize}`,
  );
  console.log(
    `[AsciiFrameParser]   offset 16 (checksum):     0x${checksum.toString(16)}`,
  );
  console.log(
    `[AsciiFrameParser]   offset 20 (flags):        0x${flags.toString(16)}`,
  );
  ...
  console.log(
    `[AsciiFrameParser] Header complete: ${width}x${height}, original=${originalSize}, compressed=${compressedSize}`,
  );
  ...
  console.log(
    `[AsciiFrameParser] Extracting frame bytes: offset=${ASCII_FRAME_HEADER_SIZE}, length=${originalSize}`,
  );
  ...
  console.log(
    `[AsciiFrameParser] Frame bytes extracted: ${frameBytes.length} bytes`,
  );
  ...
  console.log(
    `[AsciiFrameParser] Decoded to ANSI string: ${ansiString.length} characters`,
  );
