...
### WASM Module Build Fails

**Error**: "emcc: command not found"
- **Solution**: Install Emscripten
  - macOS: `brew install emscripten`
  - Linux: Check distro-specific package manager
  - Verify with: `emcc --version`

**Error**: "cmake not found" in wasm:build
- **Solution**: Ensure root ascii-chat build directory is configured
  - Run from repo root: `cmake --preset default -B build`
  - This is handled by `prebuild` script in package.json

### TypeScript Errors After WASM Changes

**Issue**: WASM types are stale after rebuilding C code
- **Solution**: Regenerate bindings and restart TypeScript server
  - Run: `bun run wasm:build`
  - Restart IDE TypeScript service (Cmd+Shift+P → "Restart TS Server")

### Playwright Tests Fail with "Error: Target closed"

**Issue**: Dev server crashed or wasn't running
- **Solution**:
  - Run `bun run dev` separately in another terminal
  - Or use `reuseExistingServer: false` in playwright.config.ts to auto-start
...
- **Solution**:
  ...
  - Or use a different port: `vite --port 3001`

### Build Fails with "max-warnings exceeded"

**Issue**: ESLint warnings detected during lint step
- **Solution**:
  - Run `bun run lint` to see all warnings
  - Fix or suppress with `// eslint-disable-next-line` (use sparingly)
  - Warnings starting with `_` are allowed: `const _unused = ...`

### WASM Memory Errors at Runtime

**Issue**: "RangeError: memory access out of bounds"
- **Solution**: Usually indicates WASM module didn't initialize properly
  - Check browser console for WASM initialization errors
  - Verify `src/wasm/dist/` WASM files exist and are correct
  - Check that `initWasmModule()` completed successfully

### Canvas/WebGL Issues in Browser

**Issue**: "WebGL context lost" or blank output
- **Solution**:
  - Refresh the page
  - Check browser console for errors
  - Test in a different browser (Chrome, Firefox)
  - Check that COEP/COOP headers are set correctly (should be in Vite config)
