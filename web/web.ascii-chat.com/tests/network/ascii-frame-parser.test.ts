...
describe('AsciiFrameParser', () => {
  ...
  it('should parse a simple uncompressed frame', () => {
    ...
    expect(result.header.originalSize).toBe(new TextEncoder().encode(frameData).length);
  ...
  it('should throw on payload shorter than header', () => {
    ...
    expect(() => parseAsciiFrame(tooShort)).toThrow('ASCII frame payload too short');
  });

  it('should throw on compressed frames', () => {
    const frameData = 'test';
    const payload = buildFramePayload(4, 1, frameData, FrameFlags.IS_COMPRESSED);

    expect(() => parseAsciiFrame(payload)).toThrow('Compressed ASCII frames not supported');
  });
