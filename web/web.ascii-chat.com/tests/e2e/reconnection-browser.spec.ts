...
test.describe("Browser Reconnection with Server Restart", () => {
  ...
  test("reconnects to Connected state when server restarts", async ({
    page,
  }) => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const isHighlighted = await connectedBtn
        .evaluate((el) => {
      ...
      console.log(
        `[E2E Test] Check Connected state ${i + 1}, highlighted: ${isHighlighted ? "yes" : "no"}`,
      );
    ...
    const errorBtnHighlighted = await errorBtn
      .evaluate((el) => {
        const style = window.getComputedStyle(el);
        return (
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const isConnecting = await connectingBtn
        .evaluate((el) => {
          const style = window.getComputedStyle(el);
          return (
            style.backgroundColor &&
      ...
      console.log(
        `[E2E Test] Status check ${i + 1}, Connecting: ${isConnecting}`,
      );
      if (isConnecting) {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      console.log(
        `[E2E Test] Reconnection check ${i + 1}, Connected: ${isHighlighted}`,
      );
      if (isHighlighted) {
