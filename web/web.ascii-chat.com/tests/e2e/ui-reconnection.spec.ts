...
test("UI: browser auto-reconnects when server restarts", async ({ page }) => {
  ...
  const startServer = async () => {
    console.log(
      `[TEST] Starting ascii-chat server on WebSocket port ${wsPort}, TCP port ${tcpPort}`,
    );
    serverProcess = spawn(
      "/Users/zfogg/src/github.com/zfogg/ascii-chat/build/bin/ascii-chat",
      [
        "server",
        "--port",
        String(tcpPort),
  ...
  if (!connected) {
    ...
    await page.screenshot({
      path: "/tmp/ui-failed-connect.png",
      fullPage: true,
    });
    await stopServer();
    return;
  ...
  console.log(
    "  Monitoring UI for 12 seconds after disconnect (100ms polling)...",
  );
  ...
  const progression = stateTimeline
    .map((s, i) => {
      const nextIdx = i + 1;
      const duration =
  ...
  const reconnectProgression = reconnectTimeline
    .map((s, i) => {
      const nextIdx = i + 1;
      const duration =
  ...
  const didReconnect =
    monitoringDone || reconnectTimeline.some((s) => s.state === "Connected");
  if (!didReconnect) {
