...
test('browser auto-reconnects when server restarts', async ({ page }) => {
  ...
  const connectionBtn = page.locator('button').filter({ hasText: 'Connection' });
  ...
  for (let i = 0; i < 20; i++) {
    ...
    isConnected = await connectedBtn.evaluate(el => {
  ...
  const isError = await errorBtn.evaluate(el => {
    const style = window.getComputedStyle(el);
    return style.backgroundColor !== 'rgb(107, 114, 128)' && style.backgroundColor !== 'rgba(0, 0, 0, 0)';
  }).catch(() => false);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    isConnecting = await connectingBtn.evaluate(el => {
      const style = window.getComputedStyle(el);
      return style.backgroundColor !== 'rgb(107, 114, 128)' && style.backgroundColor !== 'rgba(0, 0, 0, 0)';
    }).catch(() => false);
