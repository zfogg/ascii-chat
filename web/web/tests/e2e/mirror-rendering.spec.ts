...
test.describe("Mirror Page Rendering", () => {
  test("should render ASCII art in truecolor after clicking Start Webcam", async ({
    page,
  }) => {
    ...
    await expect(page.locator('button:text("Stop")')).toBeVisible({
      timeout: 10000,
    });
