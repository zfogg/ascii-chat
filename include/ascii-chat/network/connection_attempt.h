 ...
 * - WebSocket connection support (ws:// and wss://)
 * - Timeout management
 * - Crypto handshake integration (when encryption enabled)
 ...
 * - Uses crypto subsystem for optional encryption/authentication
 *
 * @date January 2026
 * @version 2.0
 */
 ...
 * - ATTEMPTING: TCP/WebSocket connection in progress
...
typedef enum {
  ...
  CONN_STATE_ATTEMPTING = 1,   ///< Attempting connection
  CONN_STATE_CONNECTED = 2,    ///< Successfully connected
...
#define CONN_TIMEOUT_TCP (3LL * NS_PER_SEC_INT) ///< TCP/WebSocket timeout (3s)
...
/**
 * @brief Context for connection attempts (TCP or WebSocket)
 *
 * Manages connection state including:
 * - State machine tracking (current state, previous state)
 * - Timeout management
 * - Transport instance (TCP or WebSocket)
 ...
 * @return Static string representation (e.g., "CONNECTED")
 */
 ...
 *
 * Closes and releases any active transports.
 ...
 * - Crypto initialization and handshake (when encryption enabled)
 * - ACIP transport creation
 *
 ...
 * - Crypto initialization (when encryption enabled)
 * - ACIP transport creation
 *
 * @param ctx Connection context (initialized via connection_context_init)
