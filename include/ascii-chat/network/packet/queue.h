 ...
 *
 * Pre-allocates a fixed-size array of packet nodes and maintains a lock-free free list
 * using atomic compare-and-swap operations. This eliminates per-packet malloc/free calls
 * and avoids lock contention for high-frequency packet queue operations.
 *
 * @note Pool operations are thread-safe via lock-free atomics (no mutexes).
 *
