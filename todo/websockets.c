...
static int read_all_timeout(int fd, void* buf, size_t n, int timeout_ms) {
    ...
    while (got < n) {
        ...
        if (rdy <= 0) return rdy == 0 ? -2 : -1;
        ssize_t r = recv(fd, p + got, n - got, 0);
        if (r == 0) return -3; // EOF
        if (r < 0) { if (errno == EINTR) continue; if (errno == EAGAIN || errno == EWOULDBLOCK) continue; return -1; }
        got += (size_t)r;
    }
    return 0;
}
...
struct sha1_ctx { uint32_t h[5]; uint64_t len_bits; uint8_t buf[64]; size_t buf_len; };

static uint32_t rol(uint32_t x, int n) { return (x<<n) | (x>>(32-n)); }

static void sha1_init(struct sha1_ctx* c){ c->h[0]=0x67452301; c->h[1]=0xEFCDAB89; c->h[2]=0x98BADCFE; c->h[3]=0x10325476; c->h[4]=0xC3D2E1F0; c->len_bits=0; c->buf_len=0; }
...
static void sha1_chunk(struct sha1_ctx* c, const uint8_t* p){
    ...
    for (int i=0;i<80;i++){
        uint32_t f,k;
        if (i<20){f=(b&g)|((~b)&d);k=0x5A827999;}
        else if(i<40){f=b^g^d;k=0x6ED9EBA1;}
        else if(i<60){f=(b&g)|(b&d)|(g&d);k=0x8F1BBCDC;}
        else {f=b^g^d;k=0xCA62C1D6;}
        uint32_t temp = rol(a,5)+f+e+k+w[i];
        e=d; d=g; g=rol(b,30); b=a; a=temp;
    }
...
static void sha1_update(struct sha1_ctx* c, const void* data, size_t len){
    ...
    while(len){
        ...
        memcpy(c->buf + c->buf_len, p, n); c->buf_len += n; p+=n; len-=n;
        if (c->buf_len==64){ sha1_chunk(c, c->buf); c->buf_len=0; }
    }
}

static void sha1_final(struct sha1_ctx* c, uint8_t out[20]){
    c->buf[c->buf_len++] = 0x80;
    if (c->buf_len > 56){ while(c->buf_len<64) c->buf[c->buf_len++]=0; sha1_chunk(c,c->buf); c->buf_len=0; }
    while(c->buf_len<56) c->buf[c->buf_len++]=0;
    for(int i=7;i>=0;i--) c->buf[c->buf_len++]= (uint8_t)(c->len_bits>>(i*8));
    sha1_chunk(c,c->buf);
...
static int http_read_headers(int fd, int timeout_ms, char** out_headers) {
    ...
    if(!buf) return -1;
    for(;;){
        if (len+1024 > cap){ cap*=2; char* nb=realloc(buf,cap); if(!nb){free(buf);return -1;} buf=nb; }
        int rdy = wait_fd(fd, POLLIN, timeout_ms);
        if (rdy <= 0){ free(buf); return rdy==0?-2:-1; }
        ssize_t r = recv(fd, buf+len, 1024, 0);
        if (r<=0){ free(buf); return -1; }
        len += (size_t)r; buf[len]='\0';
        if (strstr(buf, "\r\n\r\n")) { *out_headers=buf; return (int)len; }
        if (len > (1<<20)) { free(buf); return -3; } // 1MB header cap
    }
}
...
static int ws_compute_accept(const char* sec_key_b64, char out_b64[64]){
    ...
    struct sha1_ctx c; sha1_init(&c); sha1_update(&c, concat, (size_t)n); uint8_t digest[20]; sha1_final(&c, digest);
    int enc = base64_encode(digest, 20, out_b64, 64);
    return enc < 0 ? -1 : 0;
}

static int mk_nonce_b64(char out[32]){
    uint8_t nonce[16]; if (urandom_bytes(nonce, sizeof nonce) != 0) return -1;
    return base64_encode(nonce, sizeof nonce, out, 32) < 0 ? -1 : 0;
}

int ws_connect(struct ws_conn* ws, const char* host, const char* path, const char* port, int timeout_ms) {
    memset(ws, 0, sizeof *ws); ws->fd=-1; ws->timeout_ms = timeout_ms>0?timeout_ms:10000;
    ...
    for (ai=res; ai; ai=ai->ai_next){
        ...
        if (rdy <= 0) { close(fd); fd=-1; continue; }
        int err=0; socklen_t elen=sizeof err; getsockopt(fd,SOL_SOCKET,SO_ERROR,&err,&elen);
    ...
    if (mk_nonce_b64(ws->sec_key) != 0) { close(fd); return -3; }
    ...
    if (n<=0 || (size_t)n >= sizeof req) { close(fd); return -4; }

    if (write_all_timeout(fd, req, (size_t)n, ws->timeout_ms) != 0) { close(fd); return -5; }

    char* hdrs=NULL; int hr = http_read_headers(fd, ws->timeout_ms, &hdrs);
    if (hr <= 0) { if (hdrs) free(hdrs); close(fd); return -6; }

    // Verify 101 status
    if (strncmp(hdrs, "HTTP/1.1 101", 12) != 0 && strncmp(hdrs, "HTTP/1.0 101", 12) != 0) { free(hdrs); close(fd); return -7; }

    // Check Upgrade/Connection
    const char* up = http_hdr_get(hdrs, "Upgrade");
    const char* co = http_hdr_get(hdrs, "Connection");
    if (!up || !co || strcasestr(up,"websocket")==NULL || strcasestr(co,"Upgrade")==NULL){ free(hdrs); close(fd); return -8; }

    // Validate Sec-WebSocket-Accept
    const char* acc = http_hdr_get(hdrs, "Sec-WebSocket-Accept");
    if (!acc) { free(hdrs); close(fd); return -9; }
    char expect[64]; if (ws_compute_accept(ws->sec_key, expect)!=0){ free(hdrs); close(fd); return -10; }
    // Compare up to end-of-line
    const char* eol = strstr(acc, "\r\n"); size_t alen = eol? (size_t)(eol-acc) : strlen(acc);
    if (alen != strlen(expect) || strncasecmp(acc, expect, alen)!=0) { free(hdrs); close(fd); return -11; }
    free(hdrs);

    set_nonblock(fd,false); ws->fd=fd; ws->open=true; return 0;
}

static int ws_send_frame(struct ws_conn* ws, enum ws_opcode op, const void* data, size_t len){
    if (!ws || ws->fd<0 || !ws->open) return -1;
    uint8_t hdr[14]; size_t hlen=0;
    hdr[0] = 0x80 | (uint8_t)op; // FIN=1
    if (len <= 125) { hdr[1] = 0x80 | (uint8_t)len; hlen=2; }
    else if (len <= 0xFFFF) { hdr[1] = 0x80 | 126; hdr[2]= (len>>8)&0xFF; hdr[3]= len&0xFF; hlen=4; }
    else { hdr[1] = 0x80 | 127; for (int i=0;i<8;i++) hdr[2+i] = (uint8_t)((len >> (56-8*i)) & 0xFF); hlen=10; }
    uint8_t mask[4]; if (urandom_bytes(mask,4)!=0) return -2;
    memcpy(hdr+hlen, mask, 4); hlen += 4;

    // Prepare iov: header + masked payload (mask in-place during send in chunks)
    if (write_all_timeout(ws->fd, hdr, hlen, ws->timeout_ms)!=0) return -3;

    const uint8_t* p = data; size_t off=0; uint8_t chunk[4096];
    while (off < len){
        size_t n = len - off; if (n > sizeof chunk) n = sizeof chunk;
        memcpy(chunk, p+off, n);
        for (size_t i=0;i<n;i++) chunk[i] ^= mask[(off+i)&3];
        if (write_all_timeout(ws->fd, chunk, n, ws->timeout_ms)!=0) return -4;
        off += n;
    }
    return 0;
}

int ws_send_text(struct ws_conn* ws, const void* data, size_t len){ return ws_send_frame(ws, WS_TEXT, data, len); }
int ws_send_binary(struct ws_conn* ws, const void* data, size_t len){ return ws_send_frame(ws, WS_BINARY, data, len); }

ssize_t ws_recv(struct ws_conn* ws, uint8_t* buf, size_t cap, int* opcode_out){
    if (!ws || ws->fd<0) return -1;
    uint8_t h2[2]; if (read_all_timeout(ws->fd, h2, 2, ws->timeout_ms)!=0) return -2;
    bool fin = (h2[0] & 0x80)!=0; uint8_t op = h2[0] & 0x0F; bool masked = (h2[1] & 0x80)!=0; uint64_t len = h2[1] & 0x7F;
    if (!fin && (op==WS_TEXT||op==WS_BINARY)) return -3; // simple impl: no fragmentation for data frames
    if (len==126){ uint8_t ext[2]; if (read_all_timeout(ws->fd, ext, 2, ws->timeout_ms)!=0) return -4; len = ((uint64_t)ext[0]<<8)|ext[1]; }
    else if (len==127){ uint8_t ext[8]; if (read_all_timeout(ws->fd, ext, 8, ws->timeout_ms)!=0) return -5; len=0; for(int i=0;i<8;i++) len=(len<<8)|ext[i]; }
    uint8_t mkey[4]={0}; if (masked){ if (read_all_timeout(ws->fd, mkey, 4, ws->timeout_ms)!=0) return -6; }

    if (op==WS_PING){ // read payload and reply with PONG
        size_t n = len > cap ? cap : (size_t)len; if (read_all_timeout(ws->fd, buf, (size_t)len, ws->timeout_ms)!=0) return -7; // read all even if >cap (discard extra)
        // Unmask if needed
        if (masked) for (size_t i=0;i<(size_t)len;i++) buf[i] ^= mkey[i&3];
        ws_send_frame(ws, WS_PONG, buf, (size_t)len);
        return 0; // no app payload
    }
    if (op==WS_PONG){ // consume and ignore
        size_t toskip=(size_t)len; while(toskip){ size_t n=toskip>cap?cap:toskip; if(read_all_timeout(ws->fd, buf, n, ws->timeout_ms)!=0) return -8; toskip-=n; }
        return 0;
    }
    if (op==WS_CLOSE){ // read close code+reason, reply if not sent
        uint8_t tmp[128]; size_t n=(size_t)len; if(n>sizeof tmp)n=sizeof tmp; if(read_all_timeout(ws->fd,tmp,(size_t)len,ws->timeout_ms)!=0) return -9;
        if(masked) for(size_t i=0;i<(size_t)len;i++) tmp[i]^=mkey[i&3];
        if (ws->open){ ws_send_frame(ws, WS_CLOSE, tmp, (size_t)len); ws->open=false; }
    ...
    if ((size_t)len > cap) {
        ...
        size_t left=(size_t)len; uint8_t tmp[512];
        while(left){ size_t n = left>sizeof tmp?sizeof tmp:left; if(read_all_timeout(ws->fd,tmp,n,ws->timeout_ms)!=0) return -11; left-=n; }
        return -12; // payload too large for buffer
    }
    if (read_all_timeout(ws->fd, buf, (size_t)len, ws->timeout_ms)!=0) return -13;
    if (masked) for (size_t i=0;i<(size_t)len;i++) buf[i]^=mkey[i&3];
...
int ws_close(struct ws_conn* ws, uint16_t code, const char* reason){
    if (!ws || ws->fd<0 || !ws->open) return 0;
    uint8_t tmp[128]; size_t n=0;
    if (code){ tmp[n++]=code>>8; tmp[n++]=code&0xFF; }
    if (reason){ size_t rlen=strlen(reason); if (n+rlen>sizeof tmp) rlen = sizeof tmp - n; memcpy(tmp+n, reason, rlen); n+=rlen; }
    int rc = ws_send_frame(ws, WS_CLOSE, tmp, n);
    ws->open=false; shutdown(ws->fd, SHUT_WR);
    return rc;
}

void ws_shutdown(struct ws_conn* ws){ if(ws && ws->fd>=0){ close(ws->fd); ws->fd=-1; ws->open=false; } }

#ifdef WS_MINI_DEMO
int main(void){
    struct ws_conn ws; int rc = ws_connect(&ws, "echo.websocket.events", "/", "80", 8000);
    if (rc!=0){ fprintf(stderr, "connect failed: %d\n", rc); return 1; }
    puts("connected");
    const char* msg = "hello from c23";
    ws_send_text(&ws, msg, strlen(msg));
    uint8_t buf[1024]; int op=0; ssize_t n = ws_recv(&ws, buf, sizeof buf, &op);
    if (n>0){ printf("recv opcode=%d len=%zd: %.*s\n", op, n, (int)n, buf); }
    ws_close(&ws, 1000, "bye"); ws_shutdown(&ws); return 0;
}
#endif
