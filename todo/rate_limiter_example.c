...
void test_normal_client(void) {
  ...
  client_t client = {
      .client_id = 1, .rate_limiter = multi_rate_limiter_create(), .packets_sent = 0, .packets_blocked = 0};
...
void test_abusive_client(void) {
  ...
  client_t client = {
      .client_id = 2, .rate_limiter = multi_rate_limiter_create(), .packets_sent = 0, .packets_blocked = 0};
...
void test_burst_client(void) {
  ...
  client_t client = {
      .client_id = 3, .rate_limiter = multi_rate_limiter_create(), .packets_sent = 0, .packets_blocked = 0};
...
void test_different_types(void) {
  ...
  for (int i = 0; i < 30; i++) {
    if (rate_limiter_check(token_bucket, 100))
      tb_allowed++;
    if (rate_limiter_check(sliding_window, 100))
      sw_allowed++;
    if (rate_limiter_check(fixed_window, 100))
      fw_allowed++;
  }
...
void test_statistics(void) {
  ...
  for (int second = 0; second < 3; second++) {
    ...
    printf("  Stats: %llu allowed, %llu blocked, %.2f avg rate, %llu bytes total\n",
           (unsigned long long)stats.allowed_count, (unsigned long long)stats.blocked_count, stats.avg_rate,
