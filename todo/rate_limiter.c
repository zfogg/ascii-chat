...
static double timeval_diff_seconds(struct timeval* newer, struct timeval* older) {
    return (newer->tv_sec - older->tv_sec) +
           (newer->tv_usec - older->tv_usec) / 1000000.0;
}
...
rate_limiter_t* rate_limiter_create(const char* name, const rate_limit_config_t* config) {
    ...
    switch (config->type) {
        case RATE_LIMIT_TOKEN_BUCKET:
            token_bucket_init(&limiter->impl.token_bucket,
        ...
        case RATE_LIMIT_SLIDING_WINDOW:
            sliding_window_init(&limiter->impl.sliding_window,
                              config->window_seconds,
                              config->max_requests_per_window);
            break;

        case RATE_LIMIT_FIXED_WINDOW:
            fixed_window_init(&limiter->impl.fixed_window,
    ...
    switch (limiter->type) {
        case RATE_LIMIT_TOKEN_BUCKET:
            snprintf(status, sizeof(status),
                    "TokenBucket[%s]: %.1f/%.1f tokens, %.1f/sec refill",
                    limiter->name,
                    limiter->impl.token_bucket.tokens,
                    limiter->impl.token_bucket.max_tokens,
                    limiter->impl.token_bucket.refill_rate);
            break;

        case RATE_LIMIT_SLIDING_WINDOW:
            snprintf(status, sizeof(status),
                    "SlidingWindow[%s]: %d/%d requests in %ds window",
                    limiter->name,
                    limiter->impl.sliding_window.count,
                    limiter->impl.sliding_window.max_requests,
                    limiter->impl.sliding_window.window_seconds);
            break;

        case RATE_LIMIT_FIXED_WINDOW:
            snprintf(status, sizeof(status),
                    "FixedWindow[%s]: %d/sec, %d/min",
                    limiter->name,
                    limiter->impl.fixed_window.requests_this_second,
                    limiter->impl.fixed_window.requests_this_minute);
            break;
    }
...
rate_limiter_t* rate_limiter_create_token_bucket(const char* name,
...
rate_limiter_t* rate_limiter_create_sliding_window(const char* name,
                                                  int window_seconds,
                                                  int max_requests) {
    rate_limit_config_t config = {
        .type = RATE_LIMIT_SLIDING_WINDOW,
...
rate_limiter_t* rate_limiter_create_fixed_window(const char* name,
                                                int per_second,
                                                int per_minute) {
    rate_limit_config_t config = {
...
bool multi_rate_limiter_check_video(multi_rate_limiter_t* multi, size_t size) {
    ...
    return rate_limiter_check(multi->video_limiter, size) &&
           rate_limiter_check(multi->bandwidth_limiter, size);
}

bool multi_rate_limiter_check_audio(multi_rate_limiter_t* multi, size_t size) {
    if (!multi) return true;

    return rate_limiter_check(multi->audio_limiter, size) &&
           rate_limiter_check(multi->bandwidth_limiter, size);
}
...
bool multi_rate_limiter_check_control(multi_rate_limiter_t* multi, size_t size) {
    ...
    return rate_limiter_check(multi->control_limiter, size) &&
           rate_limiter_check(multi->bandwidth_limiter, size);
}
