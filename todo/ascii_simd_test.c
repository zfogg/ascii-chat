...
void resize_image(const rgb_pixel_t *src, int src_w, int src_h, rgb_pixel_t *dst, int dst_w, int dst_h) {
  ...
  for (int y = 0; y < dst_h; y++) {
    for (int x = 0; x < dst_w; x++) {
      ...
      int r = (int)((1 - fx) * (1 - fy) * p00->r + fx * (1 - fy) * p10->r + (1 - fx) * fy * p01->r + fx * fy * p11->r);
      int g = (int)((1 - fx) * (1 - fy) * p00->g + fx * (1 - fy) * p10->g + (1 - fx) * fy * p01->g + fx * fy * p11->g);
      int b = (int)((1 - fx) * (1 - fy) * p00->b + fx * (1 - fy) * p10->b + (1 - fx) * fy * p01->b + fx * fy * p11->b);
...
void test_neon_renderers(void) {
  ...
  for (int i = 0; i < num_sizes; i++) {
    ...
    if (fg_256_time < min_time) {
      min_time = fg_256_time;
      fastest = "256-color FG";
    }
    if (bg_true_time < min_time) {
      min_time = bg_true_time;
      fastest = "Truecolor BG";
    }
    if (fg_true_time < min_time) {
      min_time = fg_true_time;
      fastest = "Truecolor FG";
    }
...
void test_integration(void) {
  ...
  if (simd_time > 0) {
    printf("- SIMD (%s): %.5f ms per frame (%.1fx faster)\n", simd_method, simd_time * 100 / 1,
           bench.scalar_time / simd_time);
  }
