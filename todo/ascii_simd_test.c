...
typedef enum { IMAGE_SOURCE_SYNTHETIC, IMAGE_SOURCE_WEBCAM, IMAGE_SOURCE_FILE, IMAGE_SOURCE_IMG_FILES } image_source_t;
...
void test_ascii_correctness(void) {
  ...
  for (int cfg_idx = 0; cfg_idx < num_configs; cfg_idx++) {
    ...
    for (int run = 0; run < 3; run++) {
      ...
      results[run] = ascii_convert(test_image, test_configs[cfg_idx].width, test_configs[cfg_idx].height,
                                   test_configs[cfg_idx].color, test_configs[cfg_idx].aspect_ratio,
                                   test_configs[cfg_idx].stretch, DEFAULT_ASCII_PALETTE, test_luminance_palette);
...
void test_color_correctness(void) {
  ...
  const int test_sizes[][2] = {{40, 12}, {80, 24}, {160, 48}};
    ...
    for (int size_idx = 0; size_idx < num_sizes; size_idx++) {
      ...
      if (!scalar_result || !simd_result) {
        ...
        if (strcmp(scalar_result, simd_result) == 0) {
          ...
          size_t min_len = strlen(scalar_result) < strlen(simd_result) ? strlen(scalar_result) : strlen(simd_result);
          for (size_t i = 0; i < min_len; i++) {
            if (scalar_result[i] != simd_result[i]) {
              printf("    First diff at pos %zu: scalar=0x%02x('%c') simd=0x%02x('%c')\n", i,
                     (unsigned char)scalar_result[i], isprint(scalar_result[i]) ? scalar_result[i] : '.',
                     (unsigned char)simd_result[i], isprint(simd_result[i]) ? simd_result[i] : '.');
              break;
            }
          }
        }
...
void test_performance_benchmarks(void) {
  ...
  const struct {
    ...
    int width;
    int height;
  } test_sizes[] = {{"Terminal Small", 40, 12},
  ...
  for (int i = 0; i < num_sizes; i++) {
    ...
    if (utf8_ms / ascii_ms < 1.5) {
      ...
      printf("    ⚠️  UTF-8 impact: Moderate (1.5-3x slower)\n");
    } else {
      printf("    ❌ UTF-8 impact: Severe (>3x slower)\n");
    }
...
void test_integration(void) {
  ...
  if (bench.sse2_time > 0) {
    printf("  SSE2:       %8.3f ms/frame (%4.2fx scalar)\n", bench.sse2_time * 1000,
           bench.scalar_time / bench.sse2_time);
  }
  if (bench.ssse3_time > 0) {
    printf("  SSSE3:      %8.3f ms/frame (%4.2fx scalar)\n", bench.ssse3_time * 1000,
           bench.scalar_time / bench.ssse3_time);
  }
  if (bench.avx2_time > 0) {
    printf("  AVX2:       %8.3f ms/frame (%4.2fx scalar)\n", bench.avx2_time * 1000,
           bench.scalar_time / bench.avx2_time);
  }
  if (bench.neon_time > 0) {
    printf("  NEON:       %8.3f ms/frame (%4.2fx scalar)\n", bench.neon_time * 1000,
           bench.scalar_time / bench.neon_time);
  }
  if (bench.sve_time > 0) {
  ...
  char *ascii_output =
      ascii_convert(source_image, 40, 10, true, true, false, DEFAULT_ASCII_PALETTE, test_luminance_palette);
  if (ascii_output) {
