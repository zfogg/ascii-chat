...
int server_crypto_handshake(socket_t client_socket) {
  ...
  if (g_server_encryption_enabled && g_server_private_key.type == KEY_TYPE_ED25519) {
    ...
    if (g_server_private_key.use_ssh_agent) {
      // Mode 1: SSH agent mode - use ephemeral keys for encryption, agent for identity
      log_info("Server using SSH agent for authentication (ephemeral keys for encryption)");
      ...
      if (private_key_to_x25519(&g_server_private_key, x25519_sk) != 0) {
        log_error("Failed to convert Ed25519 server key to X25519");
        return -1;
      }

      // Override the generated ephemeral keys with our SSH key
      memcpy(client->crypto_handshake_ctx.crypto_ctx.private_key, x25519_sk, 32);
      ...
      log_info("Server using SSH key for authentication and encryption");
    }
