...
int main(int argc, const char **argv) {
  ...
  // Clang driver handles everything correctly - just use the compilation database as-is
  ...
  // Use an ArgumentsAdjuster that returns the original driver command from compile_commands.json
  // This preserves driver mode instead of converting to cc1
  auto driverModeAdjuster = [](const tooling::CommandLineArguments &args, StringRef filename) {
    // Return the original arguments as-is, which should contain the driver command
    // LibTooling will use these with the clang driver instead of converting to cc1
    return args;
  };
  tool.appendArgumentsAdjuster(driverModeAdjuster);
