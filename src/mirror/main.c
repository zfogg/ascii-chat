 ...
 * started, display ready, splash done). Runs the unified render loop in
 * synchronous mode using the capture context directly.
 *
...
int mirror_main(void) {
  ...
  session_client_like_config_t config = {
      .run_fn = mirror_run,
      .run_user_data = NULL,
      .kind = SESSION_CLIENT_LIKE_KIND_MIRROR,
