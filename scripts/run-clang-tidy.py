...
# NOTE: from https://clang.llvm.org/extra/doxygen/run-clang-tidy_8py_source.html
...
def get_tidy_invocation(
    ...
    if tmpdir is not None:
        ...
        os.close(handle)
        start.append(name)
    # Note: PCH version mismatch errors ("Error while processing") are common when
    # the build directory has stale precompiled headers. These errors don't prevent
    # clang-tidy from analyzing the code (check warnings count, not errors).
    # To fix: rebuild the project to regenerate PCH files matching the current compiler.
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Include dependency files in analysis. By default, only "
        "files in lib/, src/, or tests/ directories are analyzed.",
    )
    ...
    # Filter to only project code by default (for automated analysis)
    if not args.include_dependencies:
        # Only allow files in lib/, src/, or tests/ directories
        # Exclude paths that contain deps/, build/, or other dependency directories
        project_patterns = [
            r"[/\\]lib[/\\]",  # lib/ directory
            r"[/\\]src[/\\]",  # src/ directory
            r"[/\\]tests[/\\]",  # tests/ directory
        ]
        project_filter_re = re.compile("|".join(project_patterns))
        # Exclude paths that contain dependency directories
        dependency_patterns = [
            r"[/\\]deps[/\\]",  # deps/ directory
            r"[/\\]build[/\\]",  # build/ directory
        ]
        dependency_filter_re = re.compile("|".join(dependency_patterns))
        files = {
            f
            for f in files
            if project_filter_re.search(f) and not dependency_filter_re.search(f)
        }
    ...
    if not args.hide_progress:
        dependency_note = (
            " (project code only: lib/, src/, tests/)" if not args.include_dependencies else ""
        )
        print(
