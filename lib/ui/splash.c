...
static void *splash_animation_thread(void *arg) {
  ...
  while (!atomic_load(&g_splash_state.should_stop) && !shutdown_is_requested()) {
    ...
    if (!first_frame) {
      log_dev_every(1000000, "[SPLASH_ANIM] Iter %d: elapsed=%llums frame=%d should_stop=%d", iteration_count,
    ...
    if (!first_frame) {
      log_dev_every(1000000, "[SPLASH_ANIM] Iter %d: keyboard_enabled=%d", iteration_count, keyboard_enabled);
    }
    if (keyboard_enabled) {
    ...
    if (!first_frame) {
      log_dev_every(1000000, "[SPLASH_ANIM] Iter %d: keyboard check done", iteration_count);
    }
    ...
    if (!first_frame) {
      log_dev_every(500000, "[SPLASH_ANIM] Frame %d: Setting up header context", frame);
    }
    ...
    if (!first_frame) {
      log_dev_every(1000000, "[SPLASH_ANIM] Frame %d: About to lock update_notification_mutex", frame);
    }
    mutex_lock(&g_update_notification_mutex);
    if (!first_frame) {
      log_dev_every(1000000, "[SPLASH_ANIM] Frame %d: Locked update_notification_mutex", frame);
    }
    ...
    if (!first_frame) {
      log_dev_every(1000000, "[SPLASH_ANIM] Frame %d: Unlocked update_notification_mutex", frame);
    }
    ...
    if (should_render) {
      ...
      if (!first_frame) {
        log_dev_every(1000000, "[SPLASH_ANIM] Iter %d: terminal_screen_render() completed", iteration_count);
      }
      first_frame = false; // Mark first frame as complete
    } else {
      if (!first_frame) {
        log_dev_every(1000000, "[SPLASH_ANIM] Iter %d: Skipping render (should_render=false)", iteration_count);
      }
    }
