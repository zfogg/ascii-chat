 ...
 */
static void render_ui_status_header(frame_buffer_t *buf, terminal_size_t term_size, void *user_data) {
  const ui_status_t *status = (const ui_status_t *)user_data;
  if (!status) {
    return;
  }
