...
static void get_codec_from_extension(const char *path, const char **codec, const char **format,
                                     int *is_image, enum AVPixelFormat *pix_fmt) {
...
asciichat_error_t ffmpeg_encoder_create(const char *output_path,
    ...
    log_debug("ffmpeg_encoder_create: %s (%s, %dx%d @ %dfps, %s)",
              output_path, codec_name, width_px, height_px, fps,
    ...
    enc->sws_ctx = sws_getContext(width_px, height_px, AV_PIX_FMT_RGB24,
                                   width_px, height_px, enc->target_pix_fmt,
...
asciichat_error_t ffmpeg_encoder_write_frame(ffmpeg_encoder_t *enc,
    ...
    log_debug("ffmpeg_encoder_write_frame: frame %d, pitch=%d, dims=%dx%d",
              enc->frame_count, pitch, enc->width_px, enc->height_px);
    ...
    sws_scale(enc->sws_ctx, (const uint8_t * const *)data, linesize, 0,
              enc->height_px, enc->frame_encoded->data, enc->frame_encoded->linesize);
