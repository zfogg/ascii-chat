...
asciichat_error_t render_file_create(const char *output_path,
    ...
    asciichat_error_t fe = platform_font_resolve(raw_font, font_spec,
                                                 sizeof(font_spec), &font_is_path,
                                                 &font_data, &font_data_size);
    if (fe != ASCIICHAT_OK) {
        log_warn("render-file: font resolution failed for '%s' — using system default",
                 raw_font[0] ? raw_font : "(default)");
    }
