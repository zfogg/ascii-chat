...
// Global variable definitions for extern declarations in common.h
global_dec3_cache_t g_dec3_cache = {0};
char g_default_luminance_palette[256] = {0};
...
/**
 * @brief Initialize decimal-to-ASCII cache (0-255 → "0" to "255")
 *
 * Pre-builds a cache of ASCII representations for all byte values.
 * Used by color code generation to avoid repeated int-to-string conversions.
 */
void init_dec3(void) {
 ...
 *
 * Builds the default luminance-to-character mapping using the standard ASCII palette.
 * Used by legacy functions and fallback cases.
 */
