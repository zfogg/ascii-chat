...
char *render_ascii_neon_unified_optimized(const image_t *image, bool use_background, bool use_256color,
  ...
  STOP_TIMER_AND_LOG_EVERY(dev, 0, 3 * NS_PER_SEC_INT, "neon_utf8_cache", "NEON_UTF8_CACHE: Complete (%.2f ms)");
  ...
  build_neon_lookup_tables(utf8_cache, &tbl, &char_lut, &length_lut, &char_byte0_lut, &char_byte1_lut, &char_byte2_lut,
                           &char_byte3_lut);
  STOP_TIMER_AND_LOG_EVERY(dev, 0, 3 * NS_PER_SEC_INT, "neon_lookup_tables", "NEON_LOOKUP_TABLES: Complete (%.2f ms)");
  ...
  STOP_TIMER_AND_LOG_EVERY(dev, 0, 3 * NS_PER_SEC_INT, "neon_main_loop", "NEON_MAIN_LOOP: Complete (%.2f ms)");
  ...
  STOP_TIMER_AND_LOG_EVERY(dev, 0, 3 * NS_PER_SEC_INT, "neon_buffer_merge", "NEON_BUFFER_MERGE: Complete (%.2f ms)");
