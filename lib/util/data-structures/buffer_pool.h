...
typedef struct buffer_pool {
  ...
  _Atomic(size_t) used_bytes;      ///< Bytes currently in use
  _Atomic(size_t) peak_bytes;      ///< Peak bytes in use
  _Atomic(size_t) peak_pool_bytes; ///< Peak bytes in pool

  _Atomic(uint64_t) hits;             ///< Allocations from free list
  _Atomic(uint64_t) allocs;           ///< New buffer allocations
  _Atomic(uint64_t) returns;          ///< Buffers returned to free list
  _Atomic(uint64_t) shrink_freed;     ///< Buffers freed by shrink
  _Atomic(uint64_t) malloc_fallbacks; ///< Allocations that bypassed pool
  /** @} */
} buffer_pool_t;
