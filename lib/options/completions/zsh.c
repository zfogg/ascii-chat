...
asciichat_error_t completions_generate_zsh(FILE *output) {
  ...
  fprintf(output, "  )\n"
                  ...
                  "  if [[ ${words[2]} == -* ]]; then\n"
                  "    # Binary-level options: use state-based dispatch with _arguments\n"
                  "    # This establishes proper completion context. Without _arguments,\n"
                  "    # completion functions like _describe/compadd fail with\n"
                  "    # 'can only be called from completion function' error, causing\n"
                  "    # zsh to display 'corrections (errors: N)' messages. Using\n"
                  "    # _arguments with -n (no defaults) and -S (no short options)\n"
                  "    # properly sets up the state variable and completion context.\n"
