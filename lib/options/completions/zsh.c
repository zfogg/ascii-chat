...
asciichat_error_t completions_generate_zsh(FILE *output) {
  ...
  fprintf(output, "compdef _ascii_chat ascii-chat\n"
                  ...
                  "\n"
                  "\n"
                  "# Configure completion without approximate matching\n"
                  "zstyle ':completion:*:ascii-chat:*' completer _complete\n"
                  "# Explicitly disable approximate matching to prevent spurious 'corrections' messages\n"
                  "zstyle ':completion:*:ascii-chat:*' matcher-list 'm:{a-z}={A-Z}'\n"
                  "\n"
  ...
  fprintf(output, "}\n"
                  "\n"
                  "_ascii_chat() {\n"
                  "  # Prevent zsh from showing correction suggestions for partial matches\n"
                  "  compopt +o nosort\n"
                  "\n"
                  "  if [[ ${words[2]} == -* ]]; then\n"
