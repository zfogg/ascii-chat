 ...
 *
 * Displays comprehensive help for all server options, including:
 * - Description of server mode
 * - Positional argument format (bind addresses)
 * - Dual-stack binding examples
 * - All server-specific flags with descriptions
 ...
 * **Output Format**:
 * ```
 * Server mode: Accept client connections and broadcast mixed video grid
 *
 * Usage: ascii-chat server [OPTIONS] [address1] [address2]
 *
 * Positional Arguments (Bind Addresses):
 *   [address1] [address2]   Bind addresses (0-2 arguments)
 *                           0 args: Bind to 127.0.0.1 and ::1 (localhost)
 *                           1 arg:  Bind to this IPv4 OR IPv6 address
 *                           2 args: Bind to both (one IPv4, one IPv6)
 *
 * Examples:
 *   ascii-chat server                    # Localhost only (127.0.0.1 + ::1)
 *   ascii-chat server 0.0.0.0            # All IPv4 interfaces
 *   ascii-chat server ::                 # All IPv6 interfaces
 *   ascii-chat server 0.0.0.0 ::         # All interfaces (dual-stack)
 *   ascii-chat server 192.168.1.100 ::1  # Specific IPv4 + localhost IPv6
 *
 * Network Options:
 *   -p, --port PORT         Listening port (default: 27224)
 *
 * Access Control:
 *   --client-keys KEYS      Comma-separated authorized client keys
 *                           Formats: github:username, ssh-ed25519 AAAA..., ~/.ssh/client.pub
