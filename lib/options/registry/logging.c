...
const registry_entry_t g_logging_entries[] = {
...
#else
     "Set FILE as path for log file (default: $TMPDIR/ascii-chat/ on Unix or %TEMP%\\ascii-chat\\ on Windows, filenames: server.log, client.log, mirror.log, acds.log, discovery.log).",
#endif
     "LOGGING",
     "FILE",
     false,
    ...
    {"json",
     '\0',
     OPTION_TYPE_BOOL,
     offsetof(options_t, json),
     &default_json_value,
     sizeof(bool),
     "Enable JSON logging (machine-readable NDJSON format). Disabled by default (text format).",
     "LOGGING",
     NULL,
     false,
     "ASCII_CHAT_JSON",
