...
asciichat_error_t config_create_default(const char *config_path, const options_t *opts) {
  ...
  (void)fprintf(output_file, "# All options below are commented out because some configuration options\n");
  (void)fprintf(output_file, "# conflict with each other (e.g., --file vs --url, --loop vs --url).\n");
  (void)fprintf(output_file, "# Uncomment only the options you need and avoid conflicting combinations.\n");
  (void)fprintf(output_file, "#\n");
