...
static void discovery_mdns_callback(const asciichat_mdns_discovery_t *discovery, void *user_data) {
  ...
  ui_mdns_server_t *server = &state->servers[state->count];
  memset(server, 0, sizeof(ui_mdns_server_t));
...
ui_mdns_server_t *discovery_mdns_query(int timeout_ms, int max_servers, bool quiet, int *out_count) {
  ...
  *out_count = 0;
  ...
  state.servers = SAFE_MALLOC((size_t)state.capacity * sizeof(ui_mdns_server_t), ui_mdns_server_t *);
  if (!state.servers) {
    SET_ERRNO(ERROR_MEMORY, "Failed to allocate mDNS discovery server array");
    return NULL;
  }
  memset(state.servers, 0, state.capacity * sizeof(ui_mdns_server_t));
...
static void *mdns_thread_fn(void *arg) {
  ...
  ui_mdns_server_t *discovered_servers =
  ...
  for (int i = 0; i < discovered_count; i++) {
    ui_mdns_server_t *server = &discovered_servers[i];
...
asciichat_error_t discover_session_parallel(const char *session_string, const discovery_config_t *config,
  ...
  memset(&state, 0, sizeof(state));
  state.result = result;
  mutex_init(&state.lock, "mdns_resolve");
  cond_init(&state.signal, "mdns_signal");
