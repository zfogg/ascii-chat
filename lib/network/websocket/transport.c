...
/**
 * @brief Maximum receive queue size (fragment metadata buffered before reassembly)
 *
 * Power of 2 for ringbuffer optimization.
 * Queue stores metadata (pointer + size + flags) for max 512 fragments.
 * Actual fragment data is allocated separately. Fragments beyond capacity are dropped.
...
static asciichat_error_t websocket_recv(acip_transport_t *transport, void **buffer, size_t *out_len,
  ...
  while (true) {
    ...
    // Grow assembled buffer if needed (NO LOCK - work happens outside critical section)
    ...
    // Append this fragment (NO LOCK - work happens outside critical section)
...
static void websocket_destroy_impl(acip_transport_t *transport) {
  ...
  // Clear send queue and free buffered messages
