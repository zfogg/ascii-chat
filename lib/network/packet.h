...
/**
 * @brief Helper function for packet handler error handling
 *
 * Consolidates common packet reception error handling pattern:
 * - EOF: Log debug, break from loop
 * - Error: Log error, break from loop
 * - Security violation: Log error, signal exit (critical)
 *
 * Usage:
 * ```c
 * packet_recv_result_t result = receive_packet_secure(..., &envelope);
 * if (PACKET_RECV_HANDLE_ERROR(result, "operation")) {
 *     // Handled: result was error/EOF/violation
 *     // No break needed - return value indicates what to do
 *     break;  // OR continue, depending on error type
 * }
 * // Continue processing valid packet
 * ```
 *
 * @param result The packet_recv_result_t from receive_packet_secure()
 * @param context Human-readable context string for logging (e.g., "data reception")
 * @return true if error was handled (EOF/error/violation), false if success
 *
 * @ingroup network
 */
static inline bool packet_recv_handle_error(packet_recv_result_t result, const char *context) {
  if (result == PACKET_RECV_SUCCESS) {
    return false; // No error - continue processing
  }

  if (result == PACKET_RECV_EOF) {
    log_debug("Connection closed during %s", context ? context : "packet reception");
    return true;
  }

  if (result == PACKET_RECV_ERROR) {
    log_error("Failed to receive packet during %s: %s", context ? context : "operation", SAFE_STRERROR(errno));
    return true;
  }

  if (result == PACKET_RECV_SECURITY_VIOLATION) {
    log_error("SECURITY: Encryption policy violated during %s - terminating", context ? context : "packet reception");
    return true;
  }

  log_error("Unknown packet reception error (%d) during %s", result, context ? context : "operation");
  return true;
}
