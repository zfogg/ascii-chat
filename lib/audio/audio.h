...
/**
 * @brief Parsed audio batch packet header information
 *
 * Contains unpacked and validated audio batch metadata extracted from
 * PACKET_TYPE_AUDIO_BATCH packets. This structure is used by both server and
 * client protocol handlers to process batched audio frames.
 *
 * Audio Batch Packet Format:
...
/**
 * @brief Parse an audio batch packet header from raw packet data
 *
 ...
 * @param[out] out_batch Pointer to audio_batch_info_t struct to fill with parsed data
 *
 * @return ASCIICHAT_OK if parsing succeeded
 * @return ERROR_INVALID_PARAM if data is NULL or out_batch is NULL
...
/**
 * @brief Validate audio batch parameters for sanity
 *
...
/**
 * @brief Check if a sample rate is a standard/supported rate
 *
