 ...
 * @param shutdown_mutex Pointer to application's shutdown mutex (CRITICAL_SECTION)
...
/**
 * @brief Interruptible sleep for render threads using Windows condition variables
 * @param usec Number of microseconds to sleep
 *
 * Windows implementation uses SleepConditionVariableCS for responsive interruption.
...
void platform_interruptible_sleep_usec(unsigned int usec) {
  ...
  // Convert microseconds to milliseconds, minimum 1ms
  DWORD timeout_ms = (DWORD)(usec / 1000);
  if (timeout_ms < 1)
    timeout_ms = 1;
  ...
  if (!atomic_load(g_platform_should_exit)) {
