...
- Configure any preset with `-DASCIICHAT_BUILD_WITH_PANIC=ON` to enable the panic instrumentation pipeline. Examples:
  - `cmake --preset default -B build -DASCIICHAT_BUILD_WITH_PANIC=ON`
  - `cmake --build build`
- The configure step adds the panic instrumentation tool (`ascii-instr-panic`, a Clang libTooling rewriter) and the `ascii-debug-runtime` library to the build graph. When the flag is on, targets listed in `cmake/tooling/Panic.cmake` are rebuilt from a panic-instrumented tree under `build/instrumented/`.
- The generated tree mirrors project layout but skips third-party code (`deps/`) and the panic instrumentation runtime itself to avoid recursive logging.
...
- `build/instrumented/<relative/path>.c` — panic-instrumented copy of each source file.
- `build/instrumented/.stamp` — sentinel touched after a successful run.
- Only files that required panic instrumentation appear; directories are created lazily alongside rewritten files.

### Panic Macro Instrumentation Controls
...
Inspect panic-instrumented files whenever you need to confirm macro expansion handling or verify that inserted snippets match expectations.

## Panic Runtime Logging

### Per-thread Log Files

- Each thread lazily allocates a runtime state block the first time it logs. Logs live in `${ASCII_PANIC_OUTPUT_DIR:-$TMPDIR:-$TEMP:/tmp}` and use the pattern `ascii-instr-<pid>-<tid>.log`.
...
`ASCII_PANIC_ONLY` is useful when you want to lock tracing to a narrow slice of the project:

- `ASCII_PANIC_ONLY=file=lib/network/*,func=enforce_*` keeps only logs whose file path matches the glob and whose function names match `enforce_*`.
- `ASCII_PANIC_ONLY=module=server:render_*` restricts logging to files under any `/server/` directory whose basename matches `render_*`.
- `ASCII_PANIC_ONLY=server:*` is shorthand for "any file living under a `/server/` directory".
...
- Build with `-fsanitize-coverage=trace-pc-guard` (or enable the equivalent CMake option) and export `ASCII_PANIC_ENABLE_COVERAGE=1`.
...
- Combine with `llvm-symbolizer` or `addr2line` in post-processing to map PCs back to `file:line`. Coverage entries respect `ASCII_PANIC_RATE` so you can throttle high-volume traces.

## Usage Workflows

### CMake-managed builds

1. Configure with panic instrumentation enabled (see above).
2. Build normally (`cmake --build build`). Compilers will consume the panic-instrumented sources transparently.
3. Export runtime filters as needed, e.g.:
   - `export ASCII_PANIC_INCLUDE=network.c`
   - `export ASCII_PANIC_OUTPUT_DIR=/tmp/ascii-instr`
...
### Manual panic instrumentation

Use the panic instrumentation helper script when experimenting with subsets of files or out-of-tree builds:

```
./cmake/tooling/run_panic_instrumentation.sh -b build -o /tmp/ascii-instrumented -- lib/audio/audio.c src/server/server.c
```
...
### Summaries with `ascii-panic-report`

- Build the helper with `cmake --build build --target ascii-panic-report`.
- By default the tool scans `${ASCII_PANIC_OUTPUT_DIR}` (or falls back to `/tmp`) and prints the most recent record per thread.
- Common flags:
...
```
./build/bin/ascii-panic-report --log-dir /tmp/ascii-instr --thread 6789
```
...
- Mark functions that must remain async-signal-safe with `ASCII_PANIC_SIGNAL_HANDLER` before the declaration:
...
- **Performance cost.** Logging every statement carries significant overhead. Narrow the panic instrumentation scope using `ASCII_PANIC_INCLUDE` or clang-tool filters when chasing specific bugs.
...
- **Thread IDs differ by platform.** Always copy the numeric ID from log headers when using `ASCII_PANIC_THREAD`; pthread IDs and system thread IDs are not interchangeable.

## Troubleshooting

- **No logs emitted:** verify the output directory exists and is writable, and double-check that filters are not over-restrictive. Clearing `ASCII_PANIC_INCLUDE` entirely disables the include filter.
- **`Refusing to overwrite` errors during panic instrumentation:** remove the existing `build/instrumented/` directory or specify a fresh output directory.
...
- **Missing include warnings:** custom extensions may be skipped by `run_panic_instrumentation.sh`. Update the script's `find` expression if you have non-standard source suffixes.
